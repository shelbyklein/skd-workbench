# Graft and live execution graphs — individual orchestration plan
...
Own both requested graph functions: Graft exploration for connected projects, and live orchestration Flow/Files views with trustworthy evidence. Skills and MCP are independent features, not prerequisites for reading an existing index or rendering run events.

| Role | Owned work |
| --- | --- |
| Integration/runtime owner | `lib/run-events.js`, provider/workflow/terminal event adapters, durable observation contract and shared-file integration |
| Graph worker | `lib/graft-view.js`, `lib/run-graph.js`, `public/knowledge-ui.js`, `public/graphs.css`, feature unit/browser tests |
| Validation worker | Independent run-event/graph integration fixtures and tests; no edits to implementation-owned files |

Sequence: freeze index/event identities and coverage → Graft worker builds explorer while runtime owner implements durable events and validation prepares fixtures → integrate actual sequential workflow Flow view → Files evidence and Graft overlay → integrate the existing multi-agent engine → full acceptance. Use at most three child slots total across concurrently executing plans.

Existing runtime dependency: Tracker Trapper `local:11A36CE8-68E3-4E7A-856B-DF530C16ACBA`, last inspected revision 16, has `WORK-MODE-03` pending for the engine, `WORK-MODE-04` pending with partial UI evidence, and `WORK-MODE-05` pending for integrated acceptance. Re-read current state before execution. Preserve its owner/IDs; coordinate exclusive writes to runtime files. Do not implement a second scheduler or take over another active run. Provenance and directed solo launch were already recorded completed as WORK-MODE-01/02; do not reset them.

`SKD-GRAPH-01/02` can ship independently. GRAPH-03/04 can use the existing sequential runner. GRAPH-06 requires the actual engine and structured worker events, not a simulated topology. Preserve `SKD-GRAPH-01` through `SKD-GRAPH-07`. Navigation owner supplies shared routes/allowlists/cache wiring on integration.

Validate read success, denied edit, completed write, rename/new file, unattributed external change, distinct worktrees, actual dependency/review/retry/cancellation, reconnect/replay and missing telemetry. Report index readability, fixture events and actual provider activity separately.
...
Inspection baseline: `/Users/shelbyklein/Vibes/skd-workbench`, `codex/github-issues`, HEAD `d7edd1d` with substantial pending unrelated work and an additional detached settings worktree. Recheck status/worktrees before implementing; preserve all changes. Read `AGENTS.md`, `README.md`, `VALIDATION.md`, `2026-09-21-issue-work-modes.md` and `2026-09-21-navigation-orchestration-plan.md`. Earlier receipts describe their time, not current acceptance.
