...
Routes include `/#home`, `/#project/<project-id>`, `/#knowledge`, `/#knowledge/<project-id>`, `/#agents`, `/#agents/<project-id>` (legacy `/#playbooks` routes remain aliases), `/#skills`, `/#skills/<project-id>`, `/#connections`, `/#connections/<project-id>` and `/#workflows/<project-id>`. Browser back and forward restore these views. The old `/#projects` alias and existing flow, run and history URLs still work.
...
Agents replace Playbooks as versioned global or project specializations: name, description, system prompt and supported providers. (Skill and connection references from before #20 are kept but no longer applied; sessions use each CLI's own skills and MCP.) **Provider** means Codex or Claude; model, effort, workspace and task remain launch settings. Global Agents are available across eligible projects. **Copy to project** creates an independent copy; new projects do not automatically receive an Agent default. Project defaults remain provider-specific.

A session may use the project default, choose an Agent, or choose **No specialization** while retaining legacy project policies. Exact session-only resource overrides may be empty. Preview binds profile version/prompt, managed resources, project/policies and selected instruction-file contents. Changed context blocks launch; profile edits affect future sessions only. The Agent prompt travels through native developer/system arguments without a synthesized user message. Native provider instructions may also apply. Profiles cannot grant permissions or enable MCP in restricted modes.

Issue Review, planning/solo work, quick actions and verified workspace continuation review specialization before launching. MCP exclusions require an explicit reviewed selection. Sessions and imported-chat startup retain their resource controls. Workflow-step and delegation-role specializations are deferred and explicitly rejected; issue-edit proposals remain confined.

Session details expose **Save as Agent** for active and ended sessions. Drafts copy only the frozen specialization prompt and reusable managed selections, never composed Workbench/project instructions, task text, conversation, output, credentials or workspace state. Legacy sessions start with a blank prompt and a notice. Saves preserve provenance and reject reused request keys with different payloads. Observed-only MCP selection still requires complete activity coverage.

The canonical library remains `.data/playbooks.json`, schema 2. First startup validates and migrates schema 1 with a byte-exact uniquely named `playbooks.json.schema-1-<uuid>.bak`, preserving IDs, versions, defaults and historical execution records. Old API routes remain compatible; new routes use `/api/agent-profiles`. Do not start an old binary against schema 2. To roll back, stop the server, preserve the current store, reconcile any subsequent edits, and restore the schema-1 backup before starting the old version.

### Tools: skills and MCP servers

Codex and Claude Code always use their own setup (#20). Interactive sessions, quick actions, issue reviews, project agents and the orchestrator get each CLI's user, plugin and project skills and its configured MCP servers; Workbench no longer injects a managed skill block or a per-project MCP selection, and Agents no longer pick skills or MCP servers (older picks stay stored and readable but are not applied). Worktree Claude Code sessions run your normal Claude Code with its own permission prompts. Read-only and reconcile sessions, saved workflows, briefings and issue-edit proposals stay MCP-free as before.

**Tools** (Home, or a project's sidebar) shows Codex and Claude Code side by side: MCP servers from `~/.codex/config.toml`, Claude's user and project configuration (names, transport, on/off and environment variable names — values are hidden), and skills from `~/.agents/skills`, each CLI's skills folder, Codex system skills, enabled plugins and, when a project is chosen, that project's repo skills. Reading starts nothing and changes nothing. A skill present in both CLIs is marked **both**.

**Install MCP server…** installs a local command or URL server into Codex, Claude Code or both at user scope using each CLI's own command (`codex mcp add …`, `claude mcp add -s user …`). Review shows the exact commands, the file each changes and the matching remove command before anything runs; environment values are masked there and in the output and are never stored by Workbench. Commands run without a shell and each CLI reports on its own. Removing a server stays in the terminal (`codex mcp remove <name>`, `claude mcp remove -s user <name>`).

The old Skills and Connections pages and their per-project policies are retired: `#skills` and `#connections` open Tools, and `.data/skills.json` and `.data/connections.json` stay on disk unchanged.
...
**Project details → Remove project…** takes a project out of Workbench after a confirmation. It leaves the sidebar, Home, project pickers and the orchestrator's grant; its folder, Git data, branches and worktrees are never touched, and its workflows, runs, conversation and agent settings are kept. Removal is refused while the project's agent session is live or one of its workflow runs is still open (including failed or interrupted runs that could retry), and it writes a `store.json.remove-<id>.<time>.backup.json` copy first. **System → Removed projects** lists removed projects with **Restore**, which brings the project back with the same IDs. Adding a folder that belongs to a removed project offers to restore that project instead of creating a duplicate, and a link to a removed project explains how to restore it.
...
Move the flow into a connected project and use **Run settings** to save its task, acceptance checks, workspace mode, per-step provider/model/effort, skill selection and attempt limit. **Run** reviews any assigned Agents before starting (flows without a specialization start directly); **Try flow** starts the no-model walkthrough with the same saved task. Missing task or unsupported model settings open a setup form. Tiny Tasks is preconfigured, with task/acceptance shared across its benchmark workflows. Use the optional Codex bulk-model selector to assign one model to Codex steps; Claude steps keep their settings. Labels such as Fable or Opus are not silently mapped to another provider. Saving a provider/model selection or skill override updates the corresponding agent step; previous run snapshots stay unchanged. Every retry for a step reuses its frozen skill snapshot after rechecking that those skills remain eligible.
...
- `public/playbooks-ui.js`: global and project Agent libraries, versions and defaults.
...
The existing whole-project quick action retains its earlier Git-only verification contract. The separate Work lifecycle controls described below add selected-batch readiness and accepted-check receipts. Registration creates no release number and does not change project version conventions. Purpose/notes editing and the dedicated Worktrees graph remain owned by issues #7 and #4.

### Adopt and continue an existing workspace

In **Git → Details → Branches and worktrees**, select **Adopt workspace** for an eligible isolated checkout. Review its purpose, work reference and acceptance checks, then confirm that no external writer is using it and that its current changes belong to the task. Adoption saves ownership metadata; it does not create a checkout or start an agent. Existing managed registrations keep their UUID and original context. External work gets a stable task reference without allocating a release number.

After adoption, **Continue task** opens provider/model/effort selection and a next-instruction field. It starts an interactive session in that same verified checkout, preserving dirty files and original acceptance checks. Opening the dialog launches nothing. A fresh identity/content check precedes launch; changed or expired previews require reopening after refreshing Git status.

Task associations and request intents persist in `.data/workspace-tasks.json`. Failed adoption retains its original request for an explicit retry against the unchanged workspace. Restart never resumes a provider automatically. If continuation reports an uncertain result, inspect Sessions before requesting another run. Primary/source/main/comparison-target checkouts, connected project roots, benchmarks, conflicts, active Git operations and replaced identities are refused. Submodules require manual inspection; previews are bounded to 1,000 untracked files and 16 MiB of untracked content. Continue task starts an interactive session; existing structured workflow steps/retries retain their canonical workspace identity. It does not restart a completed structured run.

## Work lifecycle

Use **Project Overview → Work lifecycle** to inspect task purpose, canonical workspace UUID, owner, acceptance criteria, current Git state, original task/acceptance, evidence and retained history. Workbench is the intended development entry point. Newly managed writing sessions register automatically; external work remains explicitly unassigned until reviewed and adopted. Separate clones and external writers are outside Workbench's execution ownership.

1. **Check project** reads bounded local Git/content observations and saves only activity metadata. First observation establishes a baseline; unchanged refreshes do not count as activity. A detected source change is timestamped when observed, not when it was edited. Opening the overview does not write activity or launch a provider. Incomplete observations remain unknown.
2. **Review work** defines required acceptance checks and dependencies, records blockers/next actions, and records/reviews check reports. User reports are visibly reported, not independently observed test runs. A clean committed task becomes Ready only with accepted checks for its current commit and criteria revision. Editing criteria, ownership, dependencies or code invalidates old acceptance without deleting it. Checks are not executed by entering a command here.
3. **Reconcile work** previews a selected batch or explicitly requests full-project reconciliation. It shows captured tips, dependency order, target and exclusions. Known unfinished exclusions need ownership and a next action; unknown work, hidden dependencies, dirty selected/target checkouts and incomplete inventory block launch. Explicit start uses the existing native-permission session and shared executor. Reopening a session and retrying a lost request do not launch another process.
4. After the session stops, record and review the combined checks against the final main commit, then **Verify final commit** for the chosen environment. The receipt independently checks selected containment, preservation of original main history, unchanged excluded files/refs and absence of newly integrated excluded tips. Process exit alone is not acceptance. Local verification, optional remote synchronization and running-version observations stay separate. Partial integration or an interrupted operation remains inspectable and is never replayed automatically.
5. **Retire workspaces** previews exact selected folders and requires an explicit confirmation. Only freshly verified inactive linked worktrees qualify. Dirty, untracked or ignored files, submodules, locks, main/primary checkouts, connected project roots and uncertain identity block removal. Preserve assets outside this action first. Removal uses `git worktree remove` without force, keeps branches and retains task/evidence history. Uncertain results are inspected, never blindly retried.

Threshold settings are shared by repository: 3 unfinished workspaces, 2 weekday days without recorded activity, and 1 weekday day retaining verified eligible work. A weekday day means 24 elapsed hours on Monday–Friday in the selected IANA timezone; no holidays or office hours are assumed. Zero disables a threshold. Warnings are soft, can be snoozed with a reason/expiry, and reappear when their triggering state changes. Overlap warnings compare changed paths, not predicted conflicts; analysis is capped at 20 records and 1,000 paths per record. Unknown activity is not certified idle.

**Metadata recovery** previews existing version/package/rule files, imports legacy numbers as immutable aliases with context, and records the project's chosen release convention. It changes metadata only. Work IDs, workspace UUIDs, exact-commit checkpoints and shipped product versions remain separate. Ordinary sessions and checkpoints allocate no release number. The migration does not rewrite a project's scripts or instructions: inspect and deliberately adapt obsolete numbering requirements before declaring that project migrated. No connected project is automatically migrated by installing this feature.

Export/import preserves stable IDs, aliases, task context and receipts. Imports remain unattached and unowned; rebind explicitly verifies the unchanged original filesystem/Git identity. Moved, missing or replaced paths require manual recovery and do not inherit ownership. Export includes artifact references/checksums, **not artifact contents or full transcripts**. Required missing artifacts block acceptance. Reports are bounded to 32 MiB stores; portable requests are capped at 16 MiB. Lifecycle history retains eight versioned backups; evidence and attention retain five. Keep a separate portable backup before larger recovery operations.

Workbench's running-source identity is captured when its server starts, including a dirty-source qualifier; later checkout changes do not alter that startup observation. Other applications can have explicitly attributed manual version/commit/artifact observations. Neither source HEAD nor a verified checkpoint proves installation or deployment. Existing PWA clients use the normal explicit **Update app** action after a shell update.

Lifecycle data lives in the configured data directory (`lifecycle*.json`, associated backups, and the existing registration/task stores). Rollback must preserve these files, Git refs, workspaces and evidence; use compatible tested backups. Do not undo merges, recreate removed folders, or delete branches automatically. The dedicated Worktrees route/graph (#4), purpose/notes editing (#7), external CI, automatic deployments, background monitoring and automatic cleanup remain separate work.

## Agents in workflows

Choose an **Agent** in an agent step's inspector or in **Run settings**: a global/project specialization, **Project default**, or **No specialization**. Existing flows retain no specialization until changed. Each workflow step selects Codex or Claude, model, and effort through the shared provider selector. A moved flow keeps its references, and incompatible project Agents must be reassigned before running.

Before execution, **Review workflow Agents** shows the current Agent versions, specialization prompts, Workbench/project instructions, managed skills and exclusions. Cancel starts nothing. Structured workflows remain MCP-free; an Agent containing connections requires confirmation to run without them. **Skills for this step** uses Agent skills by default, or project skills with no specialization; choosing a replacement set overrides the Agent's skills, including an empty set. Project exclusions still apply.

All steps' configurations freeze when the run starts. Later Agent/skill edits and default changes affect new runs only. Retries and review loops reuse the frozen text, including after restart; archived, moved, unsupported or revoked resources block before the next model launch. Attempt **Frozen configuration** shows what was delivered. Old histories remain readable; they are not backfilled with today's Agents. Saving settings, previewing, and **Try flow** never start a model. Assignment to delegation roles remains separate.
...
Use **Tools → Install MCP server…** (see *Tools: skills and MCP servers*). Workbench-owned MCP definitions created before #20 stay stored but are no longer delivered to sessions.
...
The orchestrator and project agents use your normal Codex and Claude Code setup: your settings, skills, plugins and MCP servers, plus the Workbench MCP server. The orchestrator itself has no file, shell or web tools; its Claude Code keeps only the Skill tool so your skills can load, and its Codex lists your skills but cannot open their files without a shell. (Before #20 the orchestrator's Codex used a private home in `.data/coordinator-codex`; that folder is no longer read.) Sessions end on Stop, a settings change, 30 minutes idle or a server restart and are never resumed; recent transcripts stay under **Previous sessions**.
