...
## 2026-09-21 — Project overview Git status widget

- Added a Git status widget above the existing overview widgets, with current branch/commit, staged/unstaged/untracked/conflict counts, in-progress Git operations, independent upstream and integration-target comparisons, and expandable local branch/worktree inventory. Conflicts and operations in other worktrees are visible even with the inventory collapsed.
- Added shared bounded read-only Git inventory in `lib/worktrees.js`; extracted the existing Git subprocess protections into `lib/git-read.js`. All local branches are included, not only checked-out branches. Linked/detached/missing/locked worktrees, shallow/unrelated history, partial/truncated/stale reads, unborn repositories and invalid target selections remain explicit. The separately planned Worktrees graph/diff feature and issue #4 were not taken over.
- Local reads do not contact remotes. Explicit Check remote reads advertised HTTPS/SSH tips with separate timestamps, no fetch/ref/index changes, no arbitrary transport/credential helpers or embedded URL credentials, no interactive prompts, and bounded process-group cleanup. Remote errors retain local data and label previous observations stale. Missing remote objects never produce invented ahead/behind counts. No merge readiness, tests, reviews or safe-deletion claim is inferred from ancestry.
- Source baseline `f3a8ea3`. Concurrent external work committed the initial implementation as `109f5b1` and docs as `235f22a`, then started a merge in the initial worktree. That interrupted the first broad browser run with conflict markers in server.js. Preserved that merge and continued from committed source in `/Users/shelbyklein/Vibes/skd-workbench-git-status-verify`, branch `codex/git-status-verification`. Final feature refinement commit: `b8c65ab`.
- Full Node suite passed: 109 tests. Full Chrome browser suite passed: 22 scripts. Final focused Git/API tests, widget suite and PWA suite re-passed after refinements. Coverage includes zero network calls on local reads, stale/foreign/expired snapshot rejection, concurrent remote-check deduplication, transport rejection and redaction, missing remote objects, unchanged index/refs/FETCH_HEAD, target persistence, keyboard expansion, late project responses, explicit stale recovery, conflicts in another clean checkout's inventory, and 390px overflow. Expected fixture failure logs in existing issue suites are not live-provider failures.
- Actual repository inspected through the implemented UI on a separate ephemeral-port server with a temporary data store. Real remote advertisement succeeded: the observed local main tip matched the advertised main tip and the unpublished verification branch was correctly reported as not advertised. Before/after Git refs and index hashes matched. No page errors and no mobile overflow. Inspected `output/git-status-live-widget.png`, `output/git-status-live-expanded.png`, and `output/git-status-live-mobile.png`; receipt `output/git-status-live-receipt.json`. These are timestamped observations, not an enduring claim about current main or other active worktrees.
- Shell asset explicitly allowlisted and cache bumped to `skd-shell-0.5.0-61-git-status`; project data and Git APIs remain network-only. No store migration. Tracker Trapper plan `local:D8F3D8B2-CB19-4228-9878-F762E72F3DD6`, GS-01 through GS-05, linked to this task's verified session.
- Delivery is isolated source plus fixture and temporary-server UI verification. This task did not push/merge branches, restart the installed Workbench server, resolve another task's merge, launch paid provider inference, or perform Git fetch/pull/push/checkout/cleanup through the widget. Installed-app activation and any integration with concurrent work remain separate.
