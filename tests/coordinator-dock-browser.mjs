...
try{
 ...
 const tab=name=>dock.getByRole('button',{name,exact:true});
 ...
 assert.equal(await toggle.getAttribute('aria-expanded'),'true');assert.equal(await tab('Chat').getAttribute('aria-pressed'),'true');
 ...
 await tab('Chat').click();await page.emulateMedia({colorScheme:'dark'});await page.waitForTimeout(300);await page.screenshot({path:'output/coordinator-dock-chat-dark.png',animations:'disabled'});
