...
function fixture(t){
 ...
 writeFileSync(path.join(home,'.claude.json'),JSON.stringify({mcpServers:{tracker:{command:'tt',args:['mcp'],env:{API_KEY:'hidden-value'}}},claudeAiMcpEverConnected:['claude.ai Mobbin','claude.ai Gmail',42]}));
...
test('the Tools inventory lists each CLI\'s skills and MCP servers from its own setup, redacted, without starting anything',async t=>{
 ...
 assert.deepEqual(r.claude.mcp.map(m=>[m.name,m.source,m.transport]),[['tracker','user','stdio'],['Mobbin','account','connector'],['Gmail','account','connector']],'Claude.ai connectors appear as account connectors.');assert.deepEqual(r.claude.mcp[0].environmentNames,['API_KEY']);
