...
try{
 ...
 await page.setViewportSize({width:390,height:844});await page.locator('[data-graft-tab="outline"]').click();await page.locator('[data-outline-node]').first().waitFor();assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.screenshot({path:'output/graft-explorer-mobile.png',fullPage:true});assert.deepEqual(errors,[]);assert.equal(executions,0);console.log('Graft explorer: global status, project route, keyboard tabs/list, code/context/outline, search, filters, details, neighborhood, zoom, reload, mobile and zero execution requests passed.');
