...
try{
 ...
 await page.goto(base);await page.getByRole('button',{name:'Toggle coordinator panel'}).click();await page.getByRole('button',{name:'Shell',exact:true}).click();await page.waitForFunction(()=>document.querySelector('.terminal-screen')?.textContent.includes('READY'));
 const latencies=[];
 ...
 // The restored side panel reattaches to the running shell without a click or a spawn.
 await page.reload();await page.waitForFunction(()=>document.querySelector('.terminal-status')?.textContent==='running');assert.equal(spawns,1);assert.equal(input.join(''),sent);
