...
try{
 ...
 await widget.getByText('On main',{exact:true}).waitFor();assert.match(await widget.textContent(),/Clean/);assert.match(await widget.textContent(),/1 branch with commits absent from main/);assert.match(await widget.textContent(),/1 dirty worktree/);assert.equal(checks,0);
 ...
 await page.reload();await widget.getByText('Against feature/review',{exact:true}).waitFor();assert.equal(await widget.locator('[data-git-action="target"]').inputValue(),'refs/heads/feature/review');
 await widget.locator('[data-git-action="target"]').selectOption('');await widget.getByText('Against main',{exact:true}).waitFor();
 fail=true;await widget.getByRole('button',{name:'Check remote',exact:true}).click();await widget.getByText(/Remote check failed/).waitFor();assert.match(await widget.textContent(),/Previous observation — stale/);assert.match(await widget.textContent(),/On main/);
