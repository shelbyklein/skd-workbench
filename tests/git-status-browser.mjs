...
try{
 ...
 await widget.locator('.git-branch-summary strong').getByText('main',{exact:true}).waitFor();assert.equal(await widget.locator('.git-detail-panel').isVisible(),false);assert.equal(await widget.locator('.git-behind-count').textContent(),'0 branches behind main');assert((await widget.boundingBox()).height<100);await widget.getByRole('button',{name:'Details',exact:true}).click();assert.match(await widget.textContent(),/Clean/);assert.match(await widget.textContent(),/1 branch with commits absent from main/);assert.match(await widget.textContent(),/1 dirty worktree/);assert.equal(checks,0);
 await widget.getByRole('button',{name:'Check remote',exact:true}).click();await widget.getByText(/Default branch: main/).waitFor();assert.equal(checks,1);assert.match(await widget.locator('.git-remote-report').textContent(),/Same commit/);
 ...
 await widget.getByRole('button',{name:'Refresh local',exact:true}).click();await widget.getByRole('button',{name:'Details',exact:true}).click();await widget.getByRole('button',{name:'1 worktree with conflicts',exact:true}).waitFor();assert.equal(await widget.locator('.git-behind-count').textContent(),'2 branches behind main');assert.equal(await widget.locator('.git-detail-panel').isVisible(),false);await page.evaluate(()=>{document.querySelector('#toast')?.remove();});await page.setViewportSize({width:1440,height:1100});await page.screenshot({path:'output/git-row-desktop.png',fullPage:true});await page.setViewportSize({width:390,height:844});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:'output/git-row-mobile.png',fullPage:true});await widget.getByRole('button',{name:'1 worktree with conflicts',exact:true}).click();assert.equal(await widget.locator('.git-inventory').getAttribute('open'),'');assert.match(await widget.locator('.git-overview-facts').textContent(),/On main.*Clean/s);
 // Missing comparison data must not be reported as an exact zero; upstream gaps alone do not invalidate main comparisons.
 await page.route('**/api/projects/'+project.id+'/git-status*',async route=>{const response=await route.fetch();const body=await response.json();body.partial=true;body.branches.find(b=>b.name==='feature/review').upstreamComparison={state:'unknown',behind:null,ahead:null};await route.fulfill({response,json:body});});
 await widget.getByRole('button',{name:'Refresh local',exact:true}).click();await widget.getByText('2 branches behind main',{exact:true}).waitFor();await page.waitForFunction(()=>!document.querySelector('[data-git-action="refresh"]')?.disabled);
 await page.unroute('**/api/projects/'+project.id+'/git-status*');
 await page.route('**/api/projects/'+project.id+'/git-status*',async route=>{const response=await route.fetch();const body=await response.json();body.branches.find(b=>b.name==='feature/review').comparison={state:'unknown',behind:null,ahead:null};await route.fulfill({response,json:body});});
 await widget.getByRole('button',{name:'Refresh local',exact:true}).click();await widget.getByText('At least 1 branch behind main · 1 unknown',{exact:true}).waitFor();
 await page.unroute('**/api/projects/'+project.id+'/git-status*');
