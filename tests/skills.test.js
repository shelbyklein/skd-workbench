...
test('stored skill policies keep project scope, exclusions and immutable snapshots (history only since #20)',t=>{
 ...
 const inherited=skills.resolveManaged(project,'codex',{mode:'inherit',skillIDs:[]});assert.deepEqual(inherited.entries.map(x=>x.id),[local.id]);const frozen=structuredClone(inherited);
 assert.deepEqual(skills.resolveManaged(project,'codex',{mode:'replace',skillIDs:[]}).entries,[]);
 assert.throws(()=>skills.resolveManaged(other,'codex',{mode:'replace',skillIDs:[local.id]}),/another project/);
...
test('skill store rejects oversized text, corrupt data and archived launch selections',t=>{
 ...
 const entry=skills.create({revision:0,scope:{kind:'global'},name:'Fine',description:'',instructions:'fine'},[project]);skills.archive(entry.id,{revision:1,archived:true});assert.throws(()=>skills.resolveManaged(project,'claude',{mode:'replace',skillIDs:[entry.id]}),/unavailable/);
...
test('launches no longer inject managed skills: every purpose resolves to each CLI\'s own skills (#20)',t=>{
 const {project,skills}=fixture(t);const entry=skills.create({revision:0,scope:{kind:'global'},name:'Kept',description:'',instructions:'Kept text'},[project]);
 skills.savePolicy({revision:1,policyRevision:0,projectID:project.id,provider:'codex',mode:'inherit',skillIDs:[entry.id],excludedIDs:[]},[project]);
 for(const purpose of ['session','workflow'])for(const provider of ['codex','claude']){const r=skills.resolve(project,provider,{mode:'replace',skillIDs:[entry.id]},{purpose});assert.equal(r.status,'native');assert.equal(r.text,'');assert.deepEqual(r.entries,[]);}
});
