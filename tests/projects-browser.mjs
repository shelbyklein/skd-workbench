...
try{
 ...
 await page.waitForFunction(()=>document.querySelector('#project-picker').selectedOptions[0].textContent==='Newton');assert.equal(await page.locator('[data-flow]').count(),0);
 const projectA=await page.getByLabel('Project',{exact:true}).inputValue();
 ...
 await page.getByRole('button',{name:'Details',exact:true}).click();await page.getByLabel('Project name',{exact:true}).fill('Newton Dev');await page.getByRole('button',{name:'Save project',exact:true}).click();
 await page.waitForFunction(()=>document.querySelector('#project-picker').selectedOptions[0].textContent==='Newton Dev');
 ...
 await page.getByRole('button',{name:'Details',exact:true}).click();await page.waitForFunction(()=>document.querySelector('#connection-details').textContent.includes('Uncommitted changes'));await page.keyboard.press('Escape');
