...
try{
 ...
 const keys=await page.evaluate(async()=>{const keys=[];for(const name of await caches.keys())for(const r of await(await caches.open(name)).keys())keys.push(new URL(r.url).pathname);return keys;});assert(!keys.some(k=>k.startsWith('/api/')));assert.equal(keys.length,10);
 ...
 const sw=path.join(publicDirectory,'sw.js');writeFileSync(sw,readFileSync(sw,'utf8').replace(/skd-shell-0\.3\.0[^']*/, 'skd-shell-test-update'));
