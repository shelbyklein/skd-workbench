...
try{
 ...
 const keys=await page.evaluate(async()=>{const keys=[];for(const name of await caches.keys())for(const r of await(await caches.open(name)).keys())keys.push(new URL(r.url).pathname);return keys;});assert(!keys.some(k=>k.startsWith('/api/')));assert.equal(keys.length,25);for(const asset of ['/agent-card.js','/planning-ui.js','/knowledge-ui.js','/skills-ui.js','/connections-ui.js','/settings-ui.js','/issues-ui.js','/terminal-ui.js','/vendor/xterm.js','/vendor/xterm.css','/vendor/fit.js','/vendor/cytoscape.js'])assert(keys.includes(asset));
