...
try{
 ...
 await page.goto(url+'/#workflows/unassigned');await page.locator('[data-flow]').first().click();await page.locator('#project-picker').waitFor();
 ...
 const keys=await page.evaluate(async()=>{const keys=[];for(const name of await caches.keys())for(const r of await(await caches.open(name)).keys())keys.push(new URL(r.url).pathname);return keys;});assert(!keys.some(k=>k.startsWith('/api/')));assert.equal(keys.length,16);for(const asset of ['/terminal-ui.js','/vendor/xterm.js','/vendor/xterm.css','/vendor/fit.js'])assert(keys.includes(asset));
 ...
 const other=await context.newPage();await other.goto(url+'/#workflows/unassigned');await other.locator('[data-flow]').first().click();await other.locator('[data-step]').first().click();await other.getByLabel('Step name',{exact:true}).fill('Other window draft');
 ...
 await page.locator('#project-picker').waitFor();await page.waitForFunction(async()=>(await caches.keys()).length===1);
