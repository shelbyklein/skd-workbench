...
try{
 ...
 const keys=await page.evaluate(async()=>{const keys=[];for(const name of await caches.keys())for(const r of await(await caches.open(name)).keys())keys.push(new URL(r.url).pathname);return keys;});assert(!keys.some(k=>k.startsWith('/api/')));assert.equal(keys.length,43);assert(keys.includes('/tools-ui.js'));assert(keys.includes('/coordinator-dock.js'));assert(keys.includes('/sidebar-texture.png'));assert(keys.includes('/project-agent-ui.js'));for(const asset of ['/controllers-ui.js','/connection-editor.js','/workspace-tasks-ui.js','/delegations-ui.js','/session-import-ui.js','/git-status-ui.js','/agent-card.js','/planning-ui.js','/knowledge-ui.js','/skills-ui.js','/connections-ui.js','/playbooks-ui.js','/settings-ui.js','/markdown.js','/issues-ui.js','/terminal-ui.js','/vendor/xterm.js','/vendor/xterm.css','/vendor/fit.js','/vendor/cytoscape.js','/vendor/marked.js','/vendor/dompurify.js'])assert(keys.includes(asset));
 mkdirSync('output',{recursive:true});await page.screenshot({path:'output/skd-pwa-offline.png',fullPage:true});
 await page.locator('#start-workbench').waitFor();
 await page.evaluate(()=>{import('/pwa.js').then(m=>{window.launcherTest=m.startWorkbench({launch:()=>{window.launcherRequested=true;}});});});
 await page.waitForFunction(()=>window.launcherRequested===true);assert.equal(await page.locator('#start-workbench').textContent(),'Starting…');
 await context.setOffline(false);await page.locator('.project-list').waitFor();
 await page.evaluate(()=>window.launcherTest);
