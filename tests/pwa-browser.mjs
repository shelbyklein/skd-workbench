...
try{
 ...
 const keys=await page.evaluate(async()=>{const keys=[];for(const name of await caches.keys())for(const r of await(await caches.open(name)).keys())keys.push(new URL(r.url).pathname);return keys;});assert(!keys.some(k=>k.startsWith('/api/')));assert.equal(keys.length,19);for(const asset of ['/settings-ui.js','/issues-ui.js','/terminal-ui.js','/vendor/xterm.js','/vendor/xterm.css','/vendor/fit.js'])assert(keys.includes(asset));
