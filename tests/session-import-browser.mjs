...
try{
 ...
 // Import lives on the Sessions page; saving opens the imported transcript.
 await page.goto(url+'/#sessions/'+project.id);await page.locator('#import-session-chat').waitFor();
 await page.locator('#import-session-chat').focus();await page.keyboard.press('Enter');await page.getByRole('heading',{name:'Import chat',exact:true}).waitFor();
 ...
 await page.getByRole('button',{name:'Import chat',exact:true}).click();await page.locator('.imported-transcript').waitFor();assert.equal(starts,0);assert.equal(await page.locator('.imported-transcript').textContent(),transcript);assert.equal(await page.evaluate(()=>window.importExecuted),undefined);
 ...
 await page.goto(url+'/#sessions/'+project.id);await page.locator('#import-session-chat').click();await page.getByLabel('Title',{exact:true}).fill('Mobile handoff');await page.getByLabel('Import from').selectOption('file');await page.getByLabel('File path',{exact:true}).fill(path.join(root,'missing.txt'));await page.getByRole('button',{name:'Import chat',exact:true}).click();await page.getByText('Cannot read that local file. Check the path and permissions.',{exact:true}).waitFor();assert.equal(await page.getByLabel('Title',{exact:true}).inputValue(),'Mobile handoff');
 ...
 await page.getByRole('button',{name:'Import chat',exact:true}).click();await page.locator('.imported-transcript').filter({hasText:'Preserve the mobile layout'}).waitFor();assert.match(await page.locator('.imported-transcript').textContent(),/Preserve the mobile layout/);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.waitForFunction(()=>!document.querySelector('#toast').classList.contains('visible'));await page.screenshot({path:'output/session-import-detail-mobile.png',fullPage:true});
 ...
 await page.setViewportSize({width:1440,height:1050});await page.emulateMedia({colorScheme:'dark'});assert.match(await page.locator('#codex-history').innerText(),/Mobile handoff/);await page.screenshot({path:'output/session-import-card-desktop.png',fullPage:true});
