...
function fixture(t,{dir=mkdtempSync(path.join(tmpdir(),'skd-brief-sched-')),clock={value:at('2026-09-22T07:59:00Z')}}={}){
 ...
 const sources=project=>({sessions:()=>[{id:`s-${project.id}`,projectID:project.id,task:'Work',status:'completed',createdAt:'2026-09-21T20:00:00Z',finishedAt:'2026-09-21T21:00:00Z'}]});
...
test('an enabled schedule runs one batch after the configured local time',async t=>{
 ...
 assert.deepEqual(await f.briefings.tick(),{date:'2026-09-22',projects:['p1','p2']});
 assert.equal(f.executor.starts.length,1,'provider runs are serialized');
 assert.deepEqual(f.briefings.view(projects[1]).status,'queued');
 assert.equal(f.briefings.view(projects[0]).latest.evidence.interval.start,'2026-09-21T08:00:00.000Z','The daily update covers the 24 hours before it runs.');
...
test('schedule time follows the configured timezone',async t=>{
 ...
 f.clock.value=at('2026-09-22T12:00:00Z');assert.equal((await f.briefings.tick()).date,'2026-09-22');f.briefings.close();
});
...
test('restart after the time generates only today and never replays missed days',async t=>{
 ...
 assert.deepEqual(result,{date:'2026-09-22',projects:['p1','p2']});
 assert.ok(restarted.reports.every(r=>r.date==='2026-09-22'));
 restarted.close();
...
test('active user execution defers the batch instead of failing it',async t=>{
 ...
 assert.deepEqual(projects.slice(0,2).map(p=>f.briefings.view(p).status),['queued','queued']);
 f.executor.busy=false;await new Promise(r=>setTimeout(r,50));
 assert.equal(f.executor.starts.length,1);assert.equal(f.briefings.view(projects[0]).status,'generating');
 f.briefings.close();
});
