...
test('HTTP import saves without inference and only explicit same-project launch sends context',async t=>{
 ...
 const server=createServer({directory:path.join(root,'data'),terminalOptions:{binaries:{codex:binary},discover:async()=>{discoveries++;return {version:'fixture',models:[{id:'fixture',efforts:['low']}]};}}});await new Promise(r=>server.listen(0,'127.0.0.1',r));t.after(async()=>{server.shutdownCodex();server.closeAllConnections();await new Promise(r=>server.close(r));rmSync(root,{recursive:true,force:true});});const url=`http://127.0.0.1:${server.address().port}`;
