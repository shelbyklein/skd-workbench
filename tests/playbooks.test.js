...
test('versioned playbook CRUD preserves exact empty selections, history, backups and idempotent creates',async()=>{
 ...
 const retried=await library.create({revision:999,name:'Ignored',scope:{kind:'global'},providers:['codex'],creationRequestKey:'request-1'},[project]);assert.equal(retried.id,created.id);assert.equal(library.data.revision,1);
