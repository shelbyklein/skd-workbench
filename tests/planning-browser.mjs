...
const server=createServer({directory:path.join(root,'data'),codexOptions:{discover:async()=>provider},terminalOptions:{binaries:{codex:binary},discover:async()=>provider},githubOptions:{inspect:async()=>({git:{remotes:[{name:'origin',webURL:'https://github.com/fixture/planning'}]}}),request:async(endpoint,method='GET',payload)=>{if(endpoint==='graphql'){assert.equal(method,'POST');assert.match(payload.query,/^query\(/);return {data:{repository:{issues:{totalCount:1}}}};}assert.equal(method,'GET');return {id:1,number:1,title:'Plan this issue',body:'Add accessible search.',state:'open',user:{login:'fixture'},labels:[],assignees:[],comments:0,updated_at:'2026-09-21T00:00:00Z'};}}});
...
try{
 ...
 await page.goto(url+'/#issues/'+project.id+'/1');await page.reload();await page.locator('.issue-work-card > summary').click();await page.getByRole('button',{name:'Open plan',exact:true}).click();await page.waitForURL(route);await page.getByRole('button',{name:'Hide',exact:true}).click();await page.goto(url+'/#system/'+project.id);await page.reload();await page.waitForFunction(()=>document.querySelector('[data-folder-repository]')?.textContent.includes('github.com/fixture/planning'));
