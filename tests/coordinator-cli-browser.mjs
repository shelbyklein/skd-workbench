...
try{
 ...
 assert.match(await row.textContent(),/Newton/);await page.locator(`[data-home-session="${project.id}"]:is([data-state=on],[data-state=working])`).waitFor();assert.ok(['on','working'].includes(await page.locator(`[data-home-session="${project.id}"]`).getAttribute('data-state')),'Home shows a dot for the live session.');await page.locator(`[data-home-project="${project.id}"]`).screenshot({path:'output/home-session-dot.png'});assert.ok(['on','working'].includes(await row.getAttribute('data-state')));await page.waitForFunction(()=>document.querySelector('[data-dock-agent]')?.dataset.state==='on',null,{timeout:10000});assert.match(await row.textContent(),/Session on/);assert.equal(await dock.locator('.dock-project').isHidden(),true);
