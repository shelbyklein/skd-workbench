...
export function mountQuickActions(host,{project,api,confirmLeave,onOpen}){
 ...
 async function launch(action){
  ...
  try{try{request=JSON.parse(sessionStorage.getItem(key));}catch{}if(!request){const agentProfile=await reviewAgentProfile({api,project,agent:state.settings.agent,mode:action==='collaborate'?'worktree':'read-only',initial:state.settings.playbook});if(!agentProfile)return;request={agentProfile,action,revision:state.revision,requestKey:crypto.randomUUID()};try{sessionStorage.setItem(key,JSON.stringify(request));}catch{}}
 ...
 async function settings(action=null){
  ...
  dialog.innerHTML=`<form><div class="dialog-head"><h2>Quick action settings</h2><button type="button" class="icon-button" data-close aria-label="Close dialog">×</button></div><p>Saved for ${esc(project.name)}. Suggestions and reconciliation use no MCP connections. Native CLI permission prompts still apply.</p><label for="quick-agent">Provider</label><select id="quick-agent" name="agent"><option value="codex">Codex</option><option value="claude">Claude</option></select><label for="quick-model">Model</label><select id="quick-model" name="model"></select><label for="quick-effort">Effort</label><select id="quick-effort" name="effort"></select><label for="quick-playbook">Agent</label><select id="quick-playbook" name="playbook"></select><p class="form-error" role="alert" data-settings-error></p><div class="dialog-actions"><button type="button" class="text-button" data-reset>Reset settings</button><button type="button" data-close>Cancel</button><button type="submit" class="primary" disabled>${action?'Save and start':'Save settings'}</button></div></form>`;
  ...
  form.onsubmit=async e=>{e.preventDefault();if(submit.disabled)return;submit.disabled=true;try{if(!playbook.value)throw Error('Choose an available Agent.');state=await api(`projects/${project.id}/quick-actions`,'PUT',{revision:state.revision,settings:{agent:agent.value,model:model.value,effort:effort.value,playbook:['inherit','legacy'].includes(playbook.value)?{mode:playbook.value}:{mode:'selected',playbookID:playbook.value}}});close();if(action)await launch(action);}catch(e){err.textContent=e.message;submit.disabled=false;}};
