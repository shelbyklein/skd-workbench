...
export async function controllerSettings(host,api,projects){
 ...
 const render=()=>{
  ...
  host.innerHTML=`<h3>Agent control (MCP)</h3><p class="field-help">Connect an external agent to the running Workbench. Access applies to the selected projects. Running workflows is a separate permission.</p>
   ...
   <fieldset><legend>Capabilities</legend><label><input type="checkbox" checked disabled> Read projects and runs</label><label><input type="checkbox" data-controller-manage checked> Create and edit workflows; reply in project conversations</label><label><input type="checkbox" data-controller-run> Run and stop owned workflows</label></fieldset>
