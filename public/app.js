...
import {loadSettings,openSettings,projectTags,hiddenProjects,tagMarkup,openProjectTags,settingsButton,showInstructions,visibleModels} from './settings-ui.js';
...
function breadcrumbs() {
  ...
  else if(view==='tools'&&!toolsProject)items.push({name:'Tools'});
  else if(view==='briefing-global')items.push({name:'Briefing'});
  ...
  if(!['projects','project','workflows-global','knowledge-global','skills-global','connections-global','playbooks-global','invalid'].includes(view)&&!(view==='tools'&&!toolsProject)&&view!=='briefing-global'){
    ...
    }else if(view==='tools'){items.push({name:'Tools'});
...
// Home and the global pages: links to the pages that span every project (projects themselves are Home's cards).
const globalViews=[['briefing','Briefing','#briefing','briefing-global'],['knowledge','Knowledge Graph','#knowledge','knowledge-global'],['tools','Tools','#tools','tools'],['playbooks','Agents','#agents','playbooks-global']];
function globalNavigation(){
 return `<nav class="project-list project-view-list" aria-label="Workbench pages">${globalViews.map(([icon,name,href,key])=>{const active=view===key&&!(key==='tools'&&toolsProject);return `<a class="project-link ${active?'active':''}" href="${href}" ${active?'aria-current="page"':''}>${viewIcon(icon)}<span>${name}</span></a>`;}).join('')}</nav>`;
}
...
function shell(content) {
  ...
  const projectScoped=!['projects','workflows-global','knowledge-global','skills-global','connections-global','playbooks-global','invalid'].includes(view)&&!(view==='tools'&&!toolsProject)&&view!=='briefing-global';
  $('#app').innerHTML=`<aside class="sidebar"><a class="brand" href="/#home" aria-label="SKD Workbench home"><img src="/icon.svg" alt=""><span>Workbench</span></a>${projectScoped?projectNavigation():globalNavigation()}<div class="sidebar-project-actions"><button class="sidebar-add-project" data-add-project>+ Add project</button></div>${!projectScoped&&data.flows.some(f=>f.projectID==='unassigned')?`<a class="unassigned-link" href="#project/unassigned" data-sidebar-project="unassigned" ${projectScoped&&projectID==='unassigned'?'aria-current="true"':''}>Unassigned workflows</a>`:''}<div class="side-bottom">${isStandalone()?'':'<button id="install-app" class="install-button">↓ Install app</button>'}<span class="local-dot"></span> Local on your Mac</div></aside><main class="${view==='flow'?'flow-editor-main':''}"><header class="topbar">${breadcrumbs()}</header>${content}</main>`;
  const heading=$('main > .page-heading'),topbar=$('main > .topbar');
  // Home has no title bar: its content starts at the top.
  if(home&&!heading){topbar?.remove();$('main')?.classList.add('no-topbar');}
  if(heading){
  ...
  // The coordinator side panel is always open (no toggle).
  ...
  footer.append(themeControl);footer.insertAdjacentHTML('beforeend',settingsButton);
  $('.sidebar').append(footer);window.workbenchTheme?.sync();
...
function bindCommon() {
  ...
  if($('[data-global-tools]'))$('[data-global-tools]').onclick=()=>confirmLeave(()=>{view='tools';toolsProject=null;selected=null;render();});
...
function render() {
  ...
  // The side panel follows the page: a project's pages show its agent; Home and global pages the coordinator.
  const scoped=!['projects','workflows-global','knowledge-global','skills-global','connections-global','playbooks-global','invalid','briefing-global'].includes(view)&&!(view==='tools'&&!toolsProject)&&projectID!=='unassigned';
  coordinatorDock?.setProject(scoped?currentProject():null);
}
...
function watchHomeSessions(){
 ...
 homeSessionTimer=setInterval(()=>{if(!document.querySelector('[data-home-session]')){clearInterval(homeSessionTimer);homeSessionTimer=null;return;}if(document.visibilityState==='visible')fillHomeSessions();},4000);
}
function renderProjects(){
 ...
 const hidden=new Set(hiddenProjects()),hiddenCount=data.projects.filter(p=>hidden.has(p.id)).length;if(!hiddenCount)showHiddenProjects=false;
 const visibleProjects=data.projects.filter(project=>project.id!=='unassigned'&&(showHiddenProjects||!hidden.has(project.id))&&(!projectTagFilters.size||tags.some(tag=>projectTagFilters.has(tag.id)&&tag.projectIDs.includes(project.id))));
 shell(`<h1 class="visually-hidden">Home</h1><section class="workflow-overview projects-section"><div class="overview-section-heading projects-toolbar"><div class="projects-title"><h2>Projects</h2><span>${data.projects.filter(p=>p.folderPath).length} connected</span></div><div class="project-tag-filters" aria-label="Filter projects by tag">${tags.map(tag=>`<button type="button" class="project-tag-filter-pill" data-project-tag-filter="${esc(tag.id)}" aria-label="Filter by ${esc(tag.name)}" aria-pressed="${projectTagFilters.has(tag.id)}">${tagMarkup(tag)}</button>`).join('')}${hiddenCount?`<button type="button" class="text-button home-hidden-toggle" data-home-show-hidden aria-pressed="${showHiddenProjects}">${showHiddenProjects?'Hide hidden':`${hiddenCount} hidden`}</button>`:''}<span class="home-view-switch" role="group" aria-label="Project layout"><button type="button" class="icon-button" data-home-layout="cards" aria-pressed="${homeLayout==='cards'}" aria-label="Cards" title="Cards"><svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"><rect x="4" y="4" width="7" height="7" rx="1.5"/><rect x="13" y="4" width="7" height="7" rx="1.5"/><rect x="4" y="13" width="7" height="7" rx="1.5"/><rect x="13" y="13" width="7" height="7" rx="1.5"/></svg></button><button type="button" class="icon-button" data-home-layout="list" aria-pressed="${homeLayout==='list'}" aria-label="List" title="List"><svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M9 6h11M9 12h11M9 18h11"/><circle cx="4.5" cy="6" r="1" fill="currentColor"/><circle cx="4.5" cy="12" r="1" fill="currentColor"/><circle cx="4.5" cy="18" r="1" fill="currentColor"/></svg></button></span><button type="button" class="icon-button home-issues-refresh" data-home-issues-refresh aria-label="Refresh issues" title="Refresh issues"><svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20 7v5h-5M4 17v-5h5"/><path d="M6.1 6.1A8 8 0 0 1 19.5 10M4.5 14a8 8 0 0 0 13.4 3.9"/></svg></button></div></div><div class="overview-grid home-project-grid${homeLayout==='list'?' home-project-list':''}">${visibleProjects.map(p=>`${(()=>{const own=tags.filter(t=>t.projectIDs.includes(p.id)),edge=tagEdge(own);return `<article class="home-project-card${hidden.has(p.id)?' is-hidden-project':''}${own.length?' has-tag-edge':''}" data-home-project="${esc(p.id)}"${edge.style}${own.length?` title="Tags: ${esc(edge.label)}"`:''}>`;})()}<header><span class="home-session-dot" data-home-session="${esc(p.id)}" hidden></span><button class="project-card text-button" data-project="${esc(p.id)}">${esc(p.name)}</button>${projectLinks(p)}<button type="button" class="project-card-menu" data-project-tags="${esc(p.id)}" aria-label="Tags and visibility for ${esc(p.name)}" aria-haspopup="dialog">⋯</button></header><div class="project-tags visually-hidden">${tags.filter(t=>t.projectIDs.includes(p.id)).map(tagMarkup).join('')}</div><p class="home-briefing" data-home-briefing="${esc(p.id)}" hidden></p><div class="home-git" data-home-git="${esc(p.id)}"><div class="home-git-head"><svg class="home-git-icon" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="18" r="2.2"/><circle cx="6" cy="6" r="2.2"/><circle cx="18" cy="8" r="2.2"/><path d="M6 8.2v7.6M18 10.2c0 4-6 3-11 5.8"/></svg><span class="home-git-branch visually-hidden">Git</span><span class="home-git-state visually-hidden"></span><div class="home-git-stats"><span class="home-git-summary">Loading…</span></div></div></div><div class="home-issue-heading"><span>Priority issues</span><span data-project-issues="${esc(p.id)}">Loading…</span></div><ul class="priority-issue-list" data-home-issues="${esc(p.id)}"><li class="widget-empty">Loading issues…</li></ul><footer><span data-home-issue-note></span><a href="#issues/${encodeURIComponent(p.id)}">All issues ↗</a></footer></article>`).join('')}</div></section>`);
 ...
 fillHomeProjectIssues();fillHomeGit();fillHomeBriefings();watchHomeSessions();
 $('[data-home-show-hidden]')&&($('[data-home-show-hidden]').onclick=()=>{showHiddenProjects=!showHiddenProjects;renderProjects();$('[data-home-show-hidden]')?.focus();});
 document.querySelectorAll('[data-home-layout]').forEach(b=>b.onclick=()=>{homeLayout=b.dataset.homeLayout;try{localStorage.setItem('skd-home-layout',homeLayout);}catch{}$('.home-project-grid').classList.toggle('home-project-list',homeLayout==='list');document.querySelectorAll('[data-home-layout]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));});
...
async function fillHomeBriefings(){
 let rows;try{rows=await api('briefings');}catch{return;}
 for(const row of rows){const host=document.querySelector(`[data-home-briefing="${CSS.escape(row.projectID)}"]`);if(!host)continue;const line=homeLine(row.briefing);host.textContent=line||'';host.hidden=!line;}
}
 ...
 async function worker(){
  while(nodes.length){
   ...
   try{
    ...
    // Branch and clean/dirty state stay for screen readers and the tooltip; uncommitted changes tint the icon.
    branch.textContent=d.branch;host.querySelector('.home-git-state').textContent=d.dirty?'Uncommitted changes':'Clean';host.classList.toggle('is-dirty',d.dirty);host.querySelector('.home-git-head').title=`${d.branch} · ${d.dirty?'Uncommitted changes':'Clean'}`;
   ...
   }catch(error){if(host.isConnected){host.querySelector('.home-git-stats').textContent='Git status unavailable';host.title=error.message;}}
  }
 }
...
async function fillHomeProjectIssues(){
 ...
 async function worker(){
  while(nodes.length){
   const host=nodes.shift();if(!host.isConnected)continue;
   const id=host.dataset.homeIssues,note=host.closest('article').querySelector('[data-home-issue-note]');
   const count=host.closest('article').querySelector('[data-project-issues]');
   try{const result=await api('projects/'+encodeURIComponent(id)+'/issue-count');if(count.isConnected)count.textContent=`${result.count} open issue${result.count===1?'':'s'}`;}
   catch(error){if(count.isConnected){count.textContent='Issues unavailable';count.title=error.message;}}
...
function applyRoute(hash=location.hash){
 ...
 else if(route[0]==='briefing'){view='briefing-global';}
 else if(['tools','skills','connections'].includes(route[0])){view='tools';toolsProject=data.projects.some(p=>p.id===route[1]&&p.id!=='unassigned')?route[1]:null;if(toolsProject)projectID=toolsProject;}
...
async function boot(){try{await reload();await loadSettings(api);try{primeAgentCache(await api('sessions'));}catch{}applyRoute();loaded=true;render();coordinatorDock?.show();}
...
function renderTools(){
 ...
 toolsView=mountTools({host:$('#tools-view'),projectID:toolsProject,projects:data.projects,api,modal,notify:toast,onProject:id=>{toolsProject=id;if(id)projectID=id;history.replaceState(null,'','#'+routePath());}});
}
