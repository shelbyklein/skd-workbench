...
function breadcrumbs() {
  ...
  if(!['projects','project','workflows-global','knowledge-global','skills-global','connections-global','playbooks-global','invalid'].includes(view)&&!(view==='tools'&&!toolsProject)){
...
function shell(content) {
  ...
  const projectScoped=!['projects','workflows-global','knowledge-global','skills-global','connections-global','playbooks-global','invalid'].includes(view)&&!(view==='tools'&&!toolsProject);
...
function renderProjects(){
 ...
 shell(`<h1 class="visually-hidden">Home</h1><section class="workflow-overview global-pages"><div class="overview-section-heading"><h2>Global pages</h2></div><div class="overview-grid global-page-grid"><button class="overview-flow" data-global-knowledge><span class="eyebrow">ALL PROJECTS</span><strong>Knowledge Graph</strong><span>Browse connected Graft indexes.</span><span class="overview-flow-footer">Open Knowledge Graph <span aria-hidden="true">↗</span></span></button><button class="overview-flow" data-global-tools><span class="eyebrow">CODEX · CLAUDE CODE</span><strong>Tools</strong><span>Skills and MCP servers each CLI can use; install MCP servers.</span><span class="overview-flow-footer">Open Tools <span aria-hidden="true">↗</span></span></button></div></section><section class="workflow-overview projects-section"><div class="overview-section-heading projects-toolbar"><div class="projects-title"><h2>Projects</h2><span>${data.projects.filter(p=>p.folderPath).length} connected</span></div><div class="project-tag-filters" aria-label="Filter projects by tag">${tags.map(tag=>`<button type="button" class="project-tag-filter-pill" data-project-tag-filter="${esc(tag.id)}" aria-label="Filter by ${esc(tag.name)}" aria-pressed="${projectTagFilters.has(tag.id)}">${tagMarkup(tag)}</button>`).join('')}</div></div><div class="overview-grid home-project-grid">${visibleProjects.map(p=>`<article class="home-project-card" data-home-project="${esc(p.id)}"><header><button class="project-card text-button" data-project="${esc(p.id)}">${esc(p.name)}</button><button type="button" class="project-card-menu" data-project-tags="${esc(p.id)}" aria-label="Edit tags for ${esc(p.name)}" aria-haspopup="dialog">⋯</button></header><div class="project-tags">${tags.filter(t=>t.projectIDs.includes(p.id)).map(tagMarkup).join('')}</div><div class="home-issue-heading"><span>Priority issues</span><span data-project-issues="${esc(p.id)}">Loading…</span></div><ul class="priority-issue-list" data-home-issues="${esc(p.id)}"><li class="widget-empty">Loading issues…</li></ul><footer><span data-home-issue-note></span><a href="#issues/${encodeURIComponent(p.id)}">All issues ↗</a></footer></article>`).join('')}</div></section>`);
 ...
 const secondary=document.createElement('details');secondary.className='home-secondary';
 secondary.innerHTML='<summary>More</summary>';
 projectsSection.before(secondary);secondary.before(projectsSection);
 secondary.append(homeAgents,$('#home-briefings'),$('.global-pages'));
...
async function fillHomeProjectIssues(){
 ...
 async function worker(){
  while(nodes.length){
   ...
   }catch(error){if(host.isConnected){host.innerHTML='<li class="widget-empty">Issues unavailable. Open All issues to retry.</li>';host.title=error.message;}}
  }
 }
...
function renderProjectOverview(){
 ...
 coordinatorDock?.setProject(project);
}
