...
export async function openCoordinatorSettings({api,modal,notify,onSaved}){
 ...
 modal('Coordinator agent',`<p class="field-help">Claude Code or Codex answers the Home and project conversations in a live session you can watch in the CLI view. It has no file or shell tools and asks in the CLI before it starts or changes work, which it can do only within each project mandate. Codex uses its own sign-in for the coordinator: the first Codex session asks you to sign in there. Changing these settings ends running sessions. Enabling creates the “Workbench coordinator” controller grant, which you can revoke in Controllers.</p>
  ...
  <label>Model<select name="model">${options(models)}</select></label>
  <label>Effort<select name="effort"></select></label>
  <fieldset class="agent-permissions"><legend>Permissions</legend><label class="agent-check"><input type="checkbox" name="fullPermissions"${current.fullPermissions?' checked':''}> Full permissions for every agent CLI</label><p class="field-help">The coordinator and project agents start without permission prompts or sandbox: project agents can edit files and run commands in their folder on their own, and the coordinator starts or changes work without asking. Applies to sessions started after saving.</p></fieldset>`,[{label:'Cancel',close:true},{label:'Save',primary:true,submit:true}],
  async form=>{const saved=await api('coordinator','PUT',{version:current.version,enabled:form.get('enabled')==='on',provider:form.get('provider'),model:form.get('model'),effort:form.get('effort'),fullPermissions:form.get('fullPermissions')==='on'});notify((saved.enabled?'Coordinator agent on.':'Coordinator agent off.')+(saved.fullPermissions?' Agents start with full permissions.':''));onSaved?.(saved);});
 const d=document.querySelector('#dialog'),provider=d.querySelector('[name=provider]'),model=d.querySelector('[name=model]'),effort=d.querySelector('[name=effort]');
