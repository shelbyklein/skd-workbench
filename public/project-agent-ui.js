...
export function mountProjectAgent(host,{project,api,modal,notify,flows,owner,onRun,onIssue,onIssues}){
 ...
 host.innerHTML=`<div class="agent-main" id="agent-main" aria-live="polite"><p class="widget-empty">Loading project agent…</p></div>
 ...
 function renderMain(){
  const m=state.mandate,main=q('#agent-main');
  ...
  const next=m?(state.next.length?`<ul class="agent-list">${state.next.map(t=>{const n=t.ref.startsWith('github:')?issueNumber(t.ref):null;return `<li class="agent-next"><span class="agent-copy"><strong>${esc(t.title||t.ref)}</strong><small>${esc(t.ref)}</small></span>${n?`<button type="button" class="text-button" data-agent-issue="${n}">Open issue</button>`:''}</li>`;}).join('')}</ul>${state.nextTotal>state.next.length?`<p class="field-help">${state.nextTotal-state.next.length} more eligible task${state.nextTotal-state.next.length===1?'':'s'} in the mandate.</p>`:''}`:'<p class="widget-empty">All eligible tasks are claimed by a run.</p>'):'<p class="widget-empty">Set an owner mandate to list eligible tasks. <button type="button" class="text-button" data-agent-mandate>Set mandate</button></p>';
  const r=state.recent;
  const recent=r?`<article class="agent-card agent-result"><span class="agent-copy"><strong>${esc(r.flowName)}</strong><small>${esc(r.task)}</small></span><span class="agent-status agent-status-${esc(r.status)}">${esc(statusLabels[r.status]||r.status)}</span><span class="agent-result-facts">${r.approvals?`${r.approvals} review${r.approvals===1?'':'s'} approved`:'No review approval recorded'}${r.workspace?.branch?` · ${esc(r.workspace.branch)} not integrated`:''}</span><button type="button" class="text-button" data-agent-run="${esc(r.id)}">View evidence</button></article>`:'<p class="widget-empty">No finished runs.</p>';
  main.innerHTML=`<section aria-labelledby="agent-decisions"><h2 id="agent-decisions">Needs your decision</h2>${decisions}</section>
  <section aria-labelledby="agent-current"><h2 id="agent-current">Current work</h2>${current}</section>
  <section aria-labelledby="agent-next"><div class="agent-section-head"><h2 id="agent-next">Up next</h2><button type="button" class="text-button" data-agent-issues>All issues</button></div>${next}</section>
  <section aria-labelledby="agent-recent"><h2 id="agent-recent">Recent result</h2>${recent}</section>`;
 }
 ...
 async function refresh(announce=false){
  ...
  catch(error){if(host.isConnected)q('#agent-main').innerHTML=`<p class="widget-empty">${esc(error.message)}</p>`;}
 }
