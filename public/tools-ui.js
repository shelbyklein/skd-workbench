...
export function mountTools({host,projectID=null,projects,api,modal,notify,onProject}){
 ...
 function column(provider){
  ...
  const mcpRows=mcp.map(m=>`<li><details><summary><strong>${esc(m.name)}</strong><span class="tools-meta">${m.source==='account'?'claude.ai connector':`${esc(m.transport)} · ${m.enabled?'on':'off'}${m.shadowed?' · overridden':''}`}</span></summary><dl class="tools-detail">${m.source==='account'?'<div><dt>Source</dt><dd>Your Claude.ai account. Connect or remove it in Claude.ai settings; Claude Code sessions use it when it is connected.</dd></div>':`<div><dt>Source</dt><dd>${esc(m.source)} · ${esc(m.sourcePath)}</dd></div>`}${m.environmentNames.length?`<div><dt>Environment</dt><dd>${esc(m.environmentNames.join(', '))}</dd></div>`:''}${m.inlineCredentials?'<div><dt>Credentials</dt><dd>Inline credential in config (hidden)</dd></div>':''}</dl></details></li>`).join('');
