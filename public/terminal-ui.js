...
export function mountTerminal({session,api,onChange,inline=null}){
 let disposed=false,timer,cursor=0,latest=session,resizeTimer;
 function syncLayout(){if(inline)return;document.body.classList.toggle('terminal-open',!!document.querySelector('.terminal-panel:not([hidden])'));}
 const previousFocus=document.activeElement;
 const endpoint=inline?.endpoint||'terminal-sessions/';
 if(!inline)document.dispatchEvent(new Event('terminal-panel-open'));
 const panel=document.createElement(inline?'section':'aside');panel.className=inline?'coordinator-terminal':'terminal-panel';panel.setAttribute('aria-label',inline?inline.title:'Agent terminal');
 ...
 else panel.innerHTML=`<header class="terminal-header"><div><span class="eyebrow">${esc(session.agent)+' · '+esc(session.model)}</span><h2>Session terminal</h2></div><div><button id="terminal-end">End session</button><button id="terminal-hide">Hide</button></div></header><p class="terminal-status" role="status">Connecting…</p><div class="terminal-screen"></div><p class="terminal-error" role="alert"></p><footer>Closing this panel keeps the session running. Changes are never merged automatically.</footer>`;
