...
function remoteSummary(remote){
  ...
  if(remote.status==='error')return remote.message+' · Previous observation stale';
  const target=remote.target;
  const label=shortRef(target?.ref);
  const result=target?.state==='equal'?label+' matches '+remote.remoteName:label+': '+comparison(target);
  return result+' · checked '+new Date(remote.checkedAt).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'});
}
...
export function mountGitStatus(host,project,api){
  ...
  function render(){
    ...
    host.innerHTML=`<header><div><h2>Git</h2></div><div class="git-header-actions">${connected?'<button type="button" class="git-text-action" data-git-action="details" aria-expanded="'+detailsOpen+'">Details</button>':''}<button type="button" data-git-action="refresh" ${disabled?'disabled':''}>${busy?'Reading…':'Refresh local'}</button></div></header>
      ...
      ${connected?`<div class="git-compact">
        <div class="git-branch-summary"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><circle cx="6" cy="18" r="3"/><circle cx="18" cy="6" r="3"/><path d="M6 15V3m0 12c0-6 9-3 9-9"/></svg><div><strong>${esc(c?.detached?'Detached HEAD':c?.branch||headline(s))}</strong><p>${esc(s.target?(c?.comparison?.state==='equal'?'Same commit as':comparison(c?.comparison)):'Comparison unavailable')} <span>${esc(s.target?.name||'')}</span></p></div></div>
        <span class="git-checkout-state">${esc(c?.available&&c?.changes&&!c.dirty?'✓ Clean checkout':changes(c))}</span>
      </div>
      ${s.summary.conflictedWorktrees||s.summary.operationWorktrees?`<button class="git-attention" type="button" data-git-action="review"><span>${s.summary.conflictedWorktrees?`${s.partial?'At least ':''}${esc(plural(s.summary.conflictedWorktrees,'worktree'))} ${s.summary.conflictedWorktrees===1?'has':'have'} conflicts`:`${esc(plural(s.summary.operationWorktrees,'worktree'))} with a Git operation in progress`}</span><span>Review →</span></button>`:''}
      ${s.stale?'<p class="git-warning">Repository changed during inspection. Refresh local.</p>':''}
      ${s.partial?'<p class="git-warning">Partial inspection. Counts may be incomplete.</p>':''}
      <div class="git-compact-footer"><span aria-live="polite">${esc(remoteSummary(s.remote))}</span><button class="git-text-action" type="button" data-git-action="check" ${disabled||!s.selectedRemote||s.stale||error?'disabled':''}>${checking?'Checking…':'Check remote'}</button></div>
      <section class="git-detail-panel" ${detailsOpen?'':'hidden'} aria-label="Git details"><div class="git-overview-facts"><div><strong class="git-headline ${c?.changes?.conflicts||c?.operations?.length?'git-warning':''}">${esc(headline(s))}</strong><p>${esc(changes(c))}</p><small>${esc(c?.commit?.slice(0,10)||'No commit')}</small></div>
      ...
      <div class="git-remote-report" aria-live="polite">${remoteMarkup(s.remote)}</div>
