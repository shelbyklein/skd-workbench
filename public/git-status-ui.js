import {openWorkspaceTask} from './workspace-tasks-ui.js';
...
function registrationMarkup(row){
 const r=row.registration;
 if(!r)return '';
 if(r.status!=='managed')return `<p class="git-muted">${esc(r.status==='unknown'?(r.message||'Registration needs inspection'):row.current?'Project checkout · no task assigned':'Unassigned workspace')}</p>`;
 return `<p><strong>${esc(r.purpose||'No purpose recorded')}</strong></p><p class="git-muted">${esc(r.classification==='benchmark'?'Benchmark workspace':'Managed workspace')} · ${esc(r.workRef)}</p><details><summary>Registration details</summary><p class="git-muted">Workspace ID: ${esc(r.id)}</p>${r.notes?`<p>${esc(r.notes)}</p>`:''}${r.sourceAccess==='other-project'?'<p class="git-muted">Origin belongs to another project.</p>':''}</details>`;
}
function headline(s){
  const c=s.current;
...
export function mountGitStatus(host,project,api,{onSession=()=>{},onLifecycle=null}={}){
  let taskDialog=null,disposed=false,opening=false;
  ...
  function render(){
    ...
    host.innerHTML=`<header class="git-row"><h2>Git</h2>
      ...
      <div class="git-header-actions">${connected?'<button type="button" class="text-button" data-git-action="details" aria-expanded="'+detailsOpen+'">Details</button>':''}<button type="button" data-git-action="refresh" ${disabled?'disabled':''}>${busy?'Reading…':'Refresh local'}</button></div></header>
      ...
      <details class="git-inventory" ${expanded?'open':''}><summary>Branches and worktrees</summary><p class="git-muted">Commit ancestry against ${esc(s.target?.name||'the selected target')}. Squash or rebase merges may retain distinct commits. Merge readiness not checked.</p><ul>${s.branches.map(branch=>`<li class="git-branch-row"><div><strong>${esc(branch.name)}</strong><span>${esc(s.target?comparison(branch.comparison):'Choose a target')}</span></div><small>${branch.worktreeIDs.length?`${branch.worktreeIDs.length} checked-out location${branch.worktreeIDs.length===1?'':'s'}`:'Not checked out'} · ${branch.upstream?`${esc(shortRef(branch.upstream))}: ${esc(comparison(branch.upstreamComparison))}`:'No upstream'}</small></li>`).join('')||'<li>No local branches with commits.</li>'}</ul><ul class="git-worktree-list">${s.worktrees.map(w=>`<li><div><strong>${esc(w.branch||(w.detached?'Detached HEAD':w.bare?'Bare repository':'No branch'))}</strong><span>${w.current?'This checkout':''}${w.locked?' · Locked':''}${w.prunable?' · Prunable':''}</span></div><code>${esc(w.path)}</code>${registrationMarkup(w)}${onLifecycle&&w.registration?.status==='managed'?`<button type="button" class="text-button" data-git-action="lifecycle" data-registration-id="${esc(w.registration.id)}">Task details</button>`:''}<p class="${w.changes?.conflicts||w.operations?.length?'git-warning':''}">${esc(w.message||changes(w))}${w.operations?.length?' · '+esc(w.operations.join(' / '))+' in progress':''}${w.stale?' · Changed during inspection':''}${w.operations===null?' · Operation state unknown':''}</p>${w.detached?`<p>${esc(comparison(w.comparison))} against ${esc(s.target?.name||'unknown target')}</p>`:''}<button type="button" data-git-action="copy" data-worktree-id="${esc(w.id)}">Copy path</button>${w.available&&!w.current&&!w.bare&&!w.prunable&&!w.locked&&!w.detached&&w.branch&&!w.changes?.conflicts&&w.operations?.length===0&&!w.stale&&!s.stale&&!s.partial&&!s.truncated&&!['main','master',s.defaultBranch,s.target?.name].includes(w.branch)&&!project.benchmark?`<button type="button" class="text-button" data-git-action="workspace-task" data-worktree-id="${esc(w.id)}">${w.taskID?'Continue task':'Adopt workspace'}</button>`:''}</li>`).join('')}</ul>${s.registrations?.length?`<h3>Retained registrations</h3><p class="git-muted">These records are not currently attached to a verified inventory row. No ownership or completion is inferred.</p><ul class="git-worktree-list">${s.registrations.map(r=>`<li><div><strong>${esc(r.purpose||'No purpose recorded')}</strong></div><code>${esc(r.destination)}</code><p>${esc(r.workRef)} · ${esc(r.state==='attached'?'Attachment needs inspection':'Registration incomplete')}${r.error?' · '+esc(r.error):''}</p>${onLifecycle?`<button type="button" class="text-button" data-git-action="lifecycle" data-registration-id="${esc(r.id)}">Task details</button>`:''}${r.state!=='attached'&&r.sourceAccess==='project'?`<button type="button" class="text-button" data-git-action="recover-registration" data-registration-id="${esc(r.id)}" ${disabled?'disabled':''}>Retry registration</button><p class="git-muted">Checks the retained folder only. Does not create a workspace or start an agent.</p>`:''}</li>`).join('')}</ul>`:''}${s.registrationsTruncated?'<p class="git-warning">Registration history limit reached.</p>':''}</details>
    ...
    host.querySelectorAll('[data-git-action="recover-registration"]').forEach(button=>button.onclick=()=>recoverRegistration(button.dataset.registrationId));
    host.querySelectorAll('[data-git-action="lifecycle"]').forEach(button=>button.onclick=()=>onLifecycle?.(button.dataset.registrationId));
    host.querySelectorAll('[data-git-action="workspace-task"]').forEach(button=>button.onclick=()=>showTask(snapshot.worktrees.find(w=>w.id===button.dataset.worktreeId)));
    if(focus)host.querySelector(`[data-git-action="${focus}"]`)?.focus({preventScroll:true});
  }
  function showTask(worktree){if(disposed||!host.isConnected||!worktree||taskDialog?.isPending())return;if(taskDialog?.isDirty()&&!window.confirm('Discard unsaved workspace task changes?'))return;taskDialog?.dispose();taskDialog=openWorkspaceTask({project,worktree,snapshot,api,onChanged:refresh,onSession});}
  async function openTask(registrationID){if(disposed||opening||checking||taskDialog?.isPending())return;if(taskDialog?.isDirty()&&!window.confirm('Discard unsaved workspace task changes?'))return;taskDialog?.dispose();taskDialog=null;opening=true;try{const current=await refresh();if(!current||disposed||!host.isConnected||error)return;const row=snapshot?.worktrees.find(w=>w.registration?.id===registrationID);if(!row)throw Error('Workspace is no longer attached. Refresh inventory and inspect its registration.');showTask(row);}catch(e){if(!disposed&&host.isConnected){error=e.message;render();}}finally{opening=false;}}
  async function recoverRegistration(id){
    if(busy||checking||!snapshot)return;
    const record=snapshot.registrations.find(r=>r.id===id);if(!record)return;
    const request=++serial;checking=true;error='';render();
    try{await api('projects/'+encodeURIComponent(project.id)+'/workspace-registration/recover','POST',{snapshotID:snapshot.snapshotID,projectVersion:snapshot.projectVersion,registrationID:id,expectedRevision:record.revision});
      if(request!==serial||!host.isConnected)return;checking=false;await refresh();
    }catch(e){if(request===serial&&host.isConnected){checking=false;error=e.message;render();}}
  }
  async function refresh(){
    ...
    try{
      ...
      if(request!==serial||!host.isConnected)return;snapshot=result;return result;
  ...
  refresh();
  return {openTask,isDirty:()=>Boolean(taskDialog?.isDirty()),isPending:()=>Boolean(opening||taskDialog?.isPending()),dispose(){disposed=true;serial++;taskDialog?.dispose();}};
}
