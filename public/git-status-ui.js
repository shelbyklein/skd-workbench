...
export function mountGitStatus(host,project,api,{onSession=()=>{}}={}){
  let taskDialog=null;
  ...
  function render(){
    ...
    host.querySelectorAll('[data-git-action="workspace-task"]').forEach(button=>button.onclick=()=>{taskDialog?.dispose();taskDialog=openWorkspaceTask({project,worktree:snapshot.worktrees.find(w=>w.id===button.dataset.worktreeId),snapshot,api,onChanged:refresh,onSession});});
  ...
  refresh();
  return {isDirty:()=>Boolean(taskDialog?.isDirty()),isPending:()=>Boolean(taskDialog?.isPending()),dispose(){serial++;taskDialog?.dispose();}};
}
