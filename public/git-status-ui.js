...
export function mountGitStatus(host,project,api){
  let snapshot=null,error='',busy=false,checking=false,serial=0,targetRef='',remoteName='',expanded=false;
  ...
  function render(){
    ...
    host.innerHTML=`<header><div><span class="eyebrow">REPOSITORY</span><h2>Git status</h2></div><button type="button" data-git-action="refresh" ${disabled?'disabled':''}>${busy?'Reading…':'Refresh local'}</button></header>
      ...
      ${connected?`<div class="git-overview-facts"><div><strong class="git-headline ${c?.changes?.conflicts||c?.operations?.length?'git-warning':''}">${esc(headline(s))}</strong><p>${esc(changes(c))}</p><small>${esc(c?.commit?.slice(0,10)||'No commit')}</small></div>
      ...
      <footer class="widget-footer"><span>Local checked ${esc(time(s.checkedAt))} · Merge readiness not checked</span>${s.remotes.find(r=>r.name===s.selectedRemote)?.webURL?`<a href="${esc(s.remotes.find(r=>r.name===s.selectedRemote).webURL)}" target="_blank" rel="noopener noreferrer">Open repository ↗</a>`:''}</footer>`:
