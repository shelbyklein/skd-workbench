...
export async function mountHomeBriefings(host,{projects,api,onProject}){
 ...
 for(let rank=0;rank<3;rank++)for(const row of present){
  ...
  suggestions.push({...s,projectID:row.projectID});
 }
 ...
 host.innerHTML=`<div class="overview-section-heading"><h2>Daily briefing</h2><span class="briefing-home-date">${esc(day(yesterday))}</span></div><div class="briefing-home">
  <article class="project-widget"><header><div><span class="eyebrow">ALL PROJECTS</span><h3>Suggested today</h3></div></header>${suggestions.length?`<ol class="briefing-suggestions">${suggestions.slice(0,6).map(s=>`<li><button type="button" class="briefing-home-link" data-briefing-project="${esc(s.projectID)}"><span class="eyebrow">${esc(names.get(s.projectID))}</span><strong>${esc(s.title)}</strong><span>${esc(s.reason)}</span></button></li>`).join('')}</ol>`:'<p class="widget-empty">No suggestions yet. Open a project to generate its briefing.</p>'}</article>
  <article class="project-widget"><header><div><span class="eyebrow">COVERAGE</span><h3>Projects</h3></div></header><ul class="briefing-project-list">${present.map(r=>{const b=r.briefing,e=b?.latest.evidence;return `<li><button type="button" class="briefing-home-link" data-briefing-project="${esc(r.projectID)}"><strong>${esc(names.get(r.projectID))}</strong><span class="briefing-state briefing-state-${esc(status(b))}">${esc(statusLabel(b))}</span>${e?`<small>${e.activity.length} observed · ${e.openLoops.length} open${Object.values(e.coverage).some(c=>c.status!=='complete')?' · some sources incomplete':''}</small>`:''}</button></li>`;}).join('')||'<li class="widget-empty">No projects.</li>'}</ul></article></div>`;
