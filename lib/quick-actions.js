 ...
 async launch(project,input,saved,record){
  try{
   ...
   if(input.action==='reconcile'){
    ...
    assert(!snapshot.stale&&!snapshot.truncated&&!snapshot.shallow&&snapshot.worktrees.every(w=>w.available&&w.operations!==null)&&snapshot.branches.every(b=>!['unknown','unrelated'].includes(b.comparison.state)),'Repository inventory is incomplete. Resolve unavailable worktrees or history before reconciliation.',409);
