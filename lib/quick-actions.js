...
export function launchSettings(value){
 ...
 assert(!value.agentProfile||!value.playbook||JSON.stringify(value.agentProfile)===JSON.stringify(value.playbook),'Conflicting Agent selections.');const raw=value.agentProfile||value.playbook||{mode:'inherit'},p={...raw,playbookID:raw.agentProfileID||raw.playbookID};
...
export class QuickActions{
 ...
 settings(project){
  ...
  const settings=last?launchSettings({...last,playbook:(last.agentContext?.agentProfile||last.agentContext?.playbook)?.status==='selected'?{mode:'selected',playbookID:(last.agentContext.agentProfile||last.agentContext.playbook).id}:{mode:'inherit'}}):null;
 ...
 async start(project,input){
  ...
  const signature=fingerprint([project.id,input.action,input.revision,input.agentProfile||null]);
 ...
 async launch(project,input,saved,record){
  try{
   ...
   assert(!this.terminals.executor.owner&&!this.terminals.active,'Finish the active session or workflow first.',409);this.terminals.executor.projectFree(project.id);
   const effective=await this.validate(project,input.agentProfile?{...saved.settings,playbook:{mode:'legacy'}}:saved.settings,mode==='reconcile'?'read-only':mode);if(input.agentProfile){delete effective.playbook;effective.agentProfile=input.agentProfile;}
