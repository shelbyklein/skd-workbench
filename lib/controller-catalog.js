...
export const controllerTools=[
 ...
 ['get_project_instructions','read',project,'Read the start of a project\'s CLAUDE.md / AGENTS.md to decide which project a request belongs to.'],
 ['get_project_agent','read',project,'Read whether the project agent is running or waiting for the user, and its provider, model and folder.'],
 ['message_project_agent','manage',{...project,text:z.string().min(1).max(16000),requestKey:id},'Hand a request or the user\'s reply to the project\'s live agent, starting it if needed. It works in the project folder with its own instructions and tools.'],
 ['stop_project_agent','run',project,'Stop the project\'s live agent session.'],
 ['create_workbench_issue','manage',{requestKey:id,title:z.string().trim().min(1).max(256),body:z.string().max(60000)},'File an issue on SKD Workbench\'s own GitHub repository: a bug, idea or request about Workbench itself, not about a project. The user approves each one. The same requestKey never files twice. Returns the issue number and URL.'],
 ['report_to_orchestrator','report',{...project,kind:z.enum(['question','action','update']),text:z.string().min(1).max(8000),requestKey:id},'Project agents only: send the user a question, an action they need to take, or a progress update. It appears in the orchestrator chat.'],
