...
export class IssueWork{
 ...
 async startReview(project,number,input){
  ...
  const payloadHash=hash({kind:'review',projectID:project.id,projectVersion:project.version,number:Number(number),sourceHash:input.sourceHash,agentProfile:input.agentProfile||null,settings,instruction:input.instruction||''});
  ...
  try{
   const terminal=await this.terminals.start({agent:settings.agent,model:settings.model,effort:settings.effort,mode:'read-only',agentProfile:input.agentProfile||{mode:'inherit'},task:'Review issue #'+source.number,initialPrompt},project,{systemInstructions,shouldLaunch:async()=>{this.validateProject(project);const current=await this.source(project,number);return current.sourceHash===source.sourceHash;}});
 ...
 async startPlanning(project,number,input){
  ...
  const payload={projectID:project.id,sourceHash:resolved.source.sourceHash,agentProfile:input.agentProfile||null,settings:resolved.effective,prompt:resolved.prompt,kind:'planning'},payloadHash=hash(payload);
  const prior=this.data.runs.find(r=>r.requestKey===input.requestKey);
...
${JSON.stringify(resolved.effective)}`;
  try{
   const terminal=await this.terminals.start({projectID:project.id,agent:record.settings.agent,model:record.settings.model,effort:record.settings.effort,mode:'worktree',agentProfile:input.agentProfile||{mode:'inherit'},task:'Plan issue #'+record.issueNumber,initialPrompt,shouldLaunch:async()=>{const source=await this.source(project,number);return source.sourceHash===record.source.sourceHash;}},project);
 ...
 async startSolo(project,number,input){
  ...
  try{const terminal=await this.terminals.start({projectID:project.id,agent:record.settings.agent,model:record.settings.model,effort:record.settings.effort,mode:'worktree',agentProfile:input.agentProfile||{mode:'inherit'},task:'Issue work',initialPrompt:this.buildPrompt(resolved,payload.instruction),shouldLaunch:async()=>{const latest=await this.source(project,number);return latest.sourceHash===record.source.sourceHash;}},project);record.terminalID=terminal.id;record.status='running';record.startedAt=now();this.persist();return structuredClone(record);}catch(e){record.status='failed';record.error=e.message;record.finishedAt=now();this.persist();throw e;}
 }
