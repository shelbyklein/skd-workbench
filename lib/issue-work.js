...
export class IssueWork{
 constructor(directory,{github,terminals,providers}={}){
  mkdirSync(directory,{recursive:true});this.github=github;this.terminals=terminals;this.providers=providers;
 ...
 async resolve(project,number,{validate=false}={}){
  ...
  return {source,plan:plan?{planID:plan.planID,version:plan.version,planHash:plan.planHash,sourceHash:plan.sourceHash,settings:plan.settings,graph:plan.graph,createdAt:plan.createdAt}:null,approval:approval?{approvedAt:approval.approvedAt,statement:approval.statement}:null,signal,defaults,effective,override:override?.settings||null,origin,readiness,canStartSolo:!!effective&&!providerError,canStartOrchestration:!!effective?.orchestration&&executableGraph&&!providerError,executableGraph,provider,providerError};
 }
 async saveSettings(project,number,input){
  ...
  const key=this.key(project,number);this.data.overrides=this.data.overrides.filter(o=>o.key!==key);
  if(input.action!=='reset'){
   ...
   if(settings.orchestration)assert(current.executableGraph,'Orchestration needs an approved executable plan.',409);
   this.data.overrides.push({key,projectID:project.id,repository:current.source.repository,issueNumber:current.source.number,sourceHash:current.source.sourceHash,settings,updatedAt:now()});
  }
