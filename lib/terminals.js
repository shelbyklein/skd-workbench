...
export function terminalArgs(r,connectionArgs=[],connectionTools=[]){
 ...
 if(reconcile&&r.agent==='claude')return ['--model',r.model,...(r.effort==='default'?[]:['--effort',r.effort]),'--safe-mode','--strict-mcp-config','--mcp-config','{"mcpServers":{}}','--settings','{"disableAllHooks":true}','--disable-slash-commands','--disallowedTools','Agent,Task','--tools','Read,Glob,Grep,Edit,Write,Bash','--permission-mode','manual',...(r.agentContext?.skills?.text?['--append-system-prompt',r.agentContext.skills.text]:[]),...directories,...prompt];
...
export class TerminalSessions{
 ...
 async start(input,project){
  ...
  try{
   ...
   const playbookContext=this.playbooks?await this.playbooks.resolveLaunch(project,input.agent,input.playbook||{mode:'legacy',skills:input.skills,connections:input.connections},{purpose:'session',mode:input.mode==='reconcile'?'read-only':input.mode}):null;
   ...
   r={id,kind:'terminal',agent:input.agent,model:input.model,effort:input.effort,mode:input.mode,task:typeof input.task==='string'&&input.task.trim()?input.task.trim():`${input.agent==='claude'?'Claude':'Codex'} session`,...(input.initialPrompt?{initialPrompt:input.initialPrompt}:{}),projectID:project.id,projectSnapshot:structuredClone(project),sourceContext:context,workingDirectory:input.reconciliation?.workingDirectory||context.folderPath,...(input.quickAction?{quickAction:structuredClone(input.quickAction)}:{}),...(input.reconciliation?{writeDirectories:input.reconciliation.writeDirectories,reconciliationResult:{status:'pending',message:'Reconciliation session is running; completion is not verified.'}}:{}),createdAt:now(),status:'preparing',output:'',offset:0,usage:null,cost:null,toolActivity:createToolActivity(input.agent,{interactive:true}),cliVersion:provider.version,cols:80,rows:24,agentContext:{playbook:playbookContext?structuredClone(playbookContext):null,skills:skillContext,connections:connectionLaunch.snapshot}};this.runs.push(r);this.persist();
   if(r.mode==='worktree'){
