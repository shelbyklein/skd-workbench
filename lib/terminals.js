...
export function terminalArgs(r,connectionArgs=[],connectionTools=[]){
 ...
 if(r.agent==='claude'){const tools=r.mode==='read-only'?['Read','Glob','Grep']:['Read','Glob','Grep','Edit','Write',...connectionTools],toolList=tools.join(',');return ['--model',r.model,...(r.effort==='default'?[]:['--effort',r.effort]),...(r.agentContext?.connections?.status==='managed'?connectionArgs:['--safe-mode','--strict-mcp-config','--mcp-config','{"mcpServers":{}}']),...(r.agentContext?.skills?.text?['--append-system-prompt',r.agentContext.skills.text]:[]),'--settings','{"disableAllHooks":true}','--disable-slash-commands','--disallowedTools','Agent,Task','--tools',toolList,'--allowedTools',toolList,'--permission-mode','dontAsk',...(r.mode==='read-only'?['--restricted']:[]),...prompt];}
 return ['--model',r.model,'-c',`model_reasoning_effort="${r.effort}"`,'-c','features.multi_agent=false',...(r.agentContext?.skills?.text?['-c',`developer_instructions=${JSON.stringify(r.agentContext.skills.text)}`]:[]),...connectionArgs,'--sandbox',r.mode==='worktree'?'workspace-write':'read-only','--ask-for-approval',r.mode==='worktree'?'on-request':'never','--no-alt-screen','--cd',r.workingDirectory,...prompt];
}
export class TerminalSessions{
 constructor(directory,executor,{spawn=pty.spawn,discover,binaries={},skills=null,connections=null}={}){
  mkdirSync(directory,{recursive:true});this.directory=realpathSync(directory);this.executor=executor;this.spawn=spawn;this.binaries=binaries;this.skills=skills;this.connections=connections;this.provider=discover||((agent)=>agent==='claude'?discoverClaude(executor.claudeBinary,{allowSignedOut:true}):executor.discover());this.active=null;this.closed=false;this.saveTimer=null;
 ...
 async start(input,project){
  ...
  try{
   ...
   const skillContext=this.skills?.resolve(project,input.agent,input.skills||{mode:'inherit',skillIDs:[]},{purpose:'session'})||{status:'empty',entries:[],text:''};
   const connectionLaunch=this.connections?await this.connections.prepareLaunch(project,input.agent,input.connections||{mode:'inherit',connectionIDs:[]},{purpose:'session',mode:input.mode}):{snapshot:{status:'excluded',connections:[],reason:'Connections are unavailable.'},args:[],env:{},tools:[],cleanup:null};a.connectionCleanup=connectionLaunch.cleanup;a.connectionArgs=connectionLaunch.args;a.connectionEnv=connectionLaunch.env;a.connectionTools=connectionLaunch.tools;
   ...
   r={id,kind:'terminal',agent:input.agent,model:input.model,effort:input.effort,mode:input.mode,task:typeof input.task==='string'&&input.task.trim()?input.task.trim():`${input.agent==='claude'?'Claude':'Codex'} session`,...(input.initialPrompt?{initialPrompt:input.initialPrompt}:{}),projectID:project.id,projectSnapshot:structuredClone(project),sourceContext:context,workingDirectory:context.folderPath,createdAt:now(),status:'preparing',output:'',offset:0,usage:null,cost:null,cliVersion:provider.version,cols:80,rows:24,agentContext:{skills:skillContext,connections:connectionLaunch.snapshot}};this.runs.push(r);this.persist();
   if(r.mode==='worktree'){
   ...
   assert(!a.interrupted&&!this.closed,'Server stopped before launch.',409);
   if(input.shouldLaunch)assert(await input.shouldLaunch(),'Issue or project changed during launch. Refresh before starting.',409);
  ...
  }catch(e){if(r){r.status=a.interrupted?'interrupted':'failed';r.error=e.message;r.finishedAt=now();this.persist();}this.connections?.cleanupLaunch(a.connectionCleanup);this.release(a);throw e;}
 }
