...
export class TerminalSessions{
 ...
 async start(input,project){
  ...
  try{
   ...
   const playbookContext=this.playbooks?await this.playbooks.resolveLaunch(project,input.agent,input.playbook||{mode:'legacy',skills:input.skills,connections:input.connections},{purpose:'session',mode:input.mode}):null;
   ...
   const connectionLaunch=this.connections?await this.connections.prepareLaunch(project,input.agent,connectionSelection,{purpose:'session',mode:input.mode}):{snapshot:{status:'excluded',connections:[],reason:'Connections are unavailable.'},args:[],env:{},tools:[],cleanup:null};a.connectionCleanup=connectionLaunch.cleanup;a.connectionArgs=connectionLaunch.args;a.connectionEnv=connectionLaunch.env;a.connectionTools=connectionLaunch.tools;
