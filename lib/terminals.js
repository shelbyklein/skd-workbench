...
export function terminalArgs(r){
 ...
 if(r.agent==='claude')return ['--model',r.model,...(r.effort==='default'?[]:['--effort',r.effort]),'--safe-mode','--strict-mcp-config','--mcp-config','{"mcpServers":{}}','--disable-slash-commands','--disallowedTools','Agent,Task',...(r.mode==='read-only'?['--restricted','--tools','Read,Glob,Grep','--allowedTools','Read,Glob,Grep','--permission-mode','dontAsk']:[]),...prompt];
 return ['--model',r.model,'-c',`model_reasoning_effort="${r.effort}"`,'-c','features.multi_agent=false','--sandbox',r.mode==='worktree'?'workspace-write':'read-only','--ask-for-approval',r.mode==='worktree'?'on-request':'never','--no-alt-screen','--cd',r.workingDirectory,...prompt];
}
