...
export function terminalArgs(r){
 ...
 if(r.agent==='claude')return ['--model',r.model,'--effort',r.effort,'--safe-mode','--strict-mcp-config','--mcp-config','{"mcpServers":{}}','--disable-slash-commands','--disallowedTools','Agent,Task',...(r.mode==='read-only'?['--restricted','--tools','Read,Glob,Grep','--allowedTools','Read,Glob,Grep','--permission-mode','dontAsk']:[]),...prompt];
