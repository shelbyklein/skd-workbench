...
export function terminalArgs(r,connectionArgs=[],connectionTools=[]){
 ...
 // Worktree sessions run the person's normal Claude Code: their settings, skills, MCP and native permission prompts (#20).
 if(r.agent==='claude'&&r.mode==='worktree')return ['--model',r.model,...(r.effort==='default'?[]:['--effort',r.effort]),...(systemInstructions?['--append-system-prompt',systemInstructions]:[]),...prompt];
...
export class TerminalSessions{
 constructor(directory,executor,{spawn=pty.spawn,discover,binaries={},skills=null,connections=null,playbooks=null}={}){
  mkdirSync(directory,{recursive:true});this.directory=realpathSync(directory);this.executor=executor;this.spawn=spawn;this.binaries=binaries;this.skills=skills;this.connections=connections;this.playbooks=playbooks;this.provider=discover||((agent)=>agent==='claude'?discoverClaude(executor.claudeBinary,{allowSignedOut:true}):executor.discover());this.events=new EventEmitter();this.events.setMaxListeners(32);this.active=null;this.closed=false;this.saveTimer=null;
 ...
 subscribe(id,listener){this.record(id);const handler=changed=>{if(!changed||changed===id)listener();};this.events.on('change',handler);return ()=>this.events.off('change',handler);}
 persist(){clearTimeout(this.saveTimer);this.saveTimer=null;writeFileSync(this.file+'.tmp',JSON.stringify(this.runs,null,2),{mode:0o600});renameSync(this.file+'.tmp',this.file);this.events.emit('change');}
 ...
 append(r,data){if(this.closed)return;r.output+=data;if(r.output.length>LIMIT){const drop=r.output.length-LIMIT;r.output=r.output.slice(drop);r.offset+=drop;}this.events.emit('change',r.id);if(!this.saveTimer)this.saveTimer=setTimeout(()=>this.persist(),500);}
 async start(input,project,internal={}){
  ...
  assert(!this.closed&&!this.active&&!this.executor.owner&&!this.executor.active&&!this.executor.starting&&!this.executor.cleaning,'Finish the active session or workflow first.',409);this.executor.projectFree(project.id);
