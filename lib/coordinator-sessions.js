...
export function projectSessionArgs(provider,{model,effort,system,prompt,server,cwd}){
 ...
 if(provider==='claude')return ['--mcp-config',JSON.stringify({mcpServers:{workbench:{type:'stdio',command:server.command,args:server.args}}}),'--append-system-prompt',system,
  ...
  '--allowedTools',...projectFreeTools.map(t=>'mcp__workbench__'+t),'--model',model,...(effort==='default'?[]:['--effort',effort]),...(prompt?['--',prompt]:[])];
 return ['-c',`mcp_servers.workbench.command=${toml(server.command)}`,'-c',`mcp_servers.workbench.args=${toml(server.args)}`,'-c','mcp_servers.workbench.default_tools_approval_mode="approve"',
...
export class CoordinatorSessions{
 ...
 start(threadKey,{provider,model,effort,binary,credentialPath,system,prompt,env={},cwd=null,project=null}){
  ...
  if(provider==='codex'&&!cwd){mkdirSync(this.codexHome,{recursive:true,mode:0o700});chmodSync(this.codexHome,0o700);writeFileSync(path.join(this.codexHome,'config.toml'),codexConfig({server,workspace:this.workspace}),{mode:0o600});childEnv.CODEX_HOME=this.codexHome;}
  const args=cwd?projectSessionArgs(provider,{model,effort,system,prompt,server,cwd}):sessionArgs(provider,{model,effort,system,prompt,server,workspace:this.workspace});
  const child=this.spawn(binary,args,{name:'xterm-256color',cols:100,rows:30,cwd:cwd||this.workspace,env:childEnv});
  const r={id,threadKey,provider,model,effort,cwd:cwd||null,project,status:'running',waiting:false,startedAt:now(),child,secret,output:'',offset:0};
