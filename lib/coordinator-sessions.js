...
export function sessionArgs(provider,{model,effort,system,prompt,server,workspace}){
 ...
 if(provider==='claude'){
  ...
  return ['--append-system-prompt',system,'--mcp-config',JSON.stringify({mcpServers:{workbench:{type:'stdio',command:server.command,args:server.args}}}),
   '--settings',JSON.stringify(settings),'--no-chrome','--permission-mode','manual','--tools','Skill',
 ...
 return ['--sandbox','read-only','--ask-for-approval','on-request','--disable','shell_tool','--disable','unified_exec','-c','features.multi_agent=false','-c','web_search="disabled"',
  ...
  ...promptTools.flatMap(t=>['-c',`mcp_servers.workbench.tools.${t}.approval_mode="prompt"`]),
...
export class CoordinatorSessions{
 ...
 start(threadKey,{provider,model,effort,binary,credentialPath,system,prompt,env={},cwd=null,project=null}){
  ...
  child.onData(data=>{if(!r.startupPrompt&&/Hooks need review|Do you trust the (?:files|contents) in this folder/.test(r.output.slice(-300)+data)){r.startupPrompt=true;r.waiting=true;}r.output+=data;if(r.output.length>this.tailChars){const trim=r.output.length-this.tailChars;r.output=r.output.slice(trim);r.offset+=trim;}this.touch(r);this.events.emit(r.id);});
