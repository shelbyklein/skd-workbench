...
// Sessions end on Stop, settings change, idle or server stop, and are never resumed.
export const freeTools=controllerTools.filter(({name,capability})=>capability==='read'||/^post_/.test(name)).map(({name})=>name);
export const promptTools=controllerTools.map(({name})=>name).filter(name=>!freeTools.includes(name));
...
export function sessionArgs(provider,{model,effort,system,prompt,server,workspace}){
 if(provider==='claude'){
  const settings={hooks:{Notification:[{matcher:'permission_prompt',hooks:[{type:'command',command:`${toml(server.command)} ${toml(hook)} waiting`}]}]}};
  return ['--system-prompt',system,'--strict-mcp-config','--mcp-config',JSON.stringify({mcpServers:{workbench:{type:'stdio',command:server.command,args:server.args}}}),
   '--setting-sources','project','--settings',JSON.stringify(settings),'--disable-slash-commands','--no-chrome','--permission-mode','manual','--tools','',
   '--allowedTools',...freeTools.map(t=>'mcp__workbench__'+t),'--model',model,...(effort==='default'?[]:['--effort',effort]),'--',prompt];
 }
 return ['--sandbox','read-only','--ask-for-approval','on-request','--disable','shell_tool','--disable','unified_exec','-c','features.multi_agent=false','-c','web_search="disabled"',
...
// Codex reads only this private home: the Workbench server, per-tool approval, the signal hook and folder trust.
...
export class CoordinatorSessions{
 constructor(directory,{spawn=pty.spawn,nodePath=process.execPath,idleMs=1800000,historyLimit=20,tailChars=262144,killDelay=3000}={}){
 ...
 summary(r){return r&&{id:r.id,threadKey:r.threadKey,provider:r.provider,model:r.model,effort:r.effort,status:r.status,waiting:!!r.waiting,startedAt:r.startedAt,endedAt:r.endedAt||null,endReason:r.endReason||null};}
 ...
 input(id,data){const r=this.find(id);assert(typeof data==='string'&&data.length>0&&data.length<=16384,'Invalid terminal input.');assert(r.status==='running','This coordinator session has ended.',409);r.waiting=false;this.touch(r);r.child.write(data);return {ok:true};}
 ...
 deliver(threadKey,text,start){
  ...
  if(r?.status==='running'){r.waiting=false;this.touch(r);r.child.write(PASTE_START+text.replace(/\x1b\[20[01]~/g,'')+PASTE_END);setTimeout(()=>{if(r.status==='running')r.child.write('\r');},60).unref?.();this.events.emit(r.id);return this.summary(r);}
  return this.start(threadKey,start());
 }
 start(threadKey,{provider,model,effort,binary,credentialPath,system,prompt,env={}}){
  ...
  if(provider==='codex'){mkdirSync(this.codexHome,{recursive:true,mode:0o700});chmodSync(this.codexHome,0o700);writeFileSync(path.join(this.codexHome,'config.toml'),codexConfig({server,workspace:this.workspace}),{mode:0o600});childEnv.CODEX_HOME=this.codexHome;}
  const child=this.spawn(binary,sessionArgs(provider,{model,effort,system,prompt,server,workspace:this.workspace}),{name:'xterm-256color',cols:100,rows:30,cwd:this.workspace,env:childEnv});
  const r={id,threadKey,provider,model,effort,status:'running',waiting:false,startedAt:now(),child,secret,output:'',offset:0};
  this.live.set(threadKey,r);this.touch(r);
  child.onData(data=>{r.output+=data;if(r.output.length>this.tailChars){const trim=r.output.length-this.tailChars;r.output=r.output.slice(trim);r.offset+=trim;}this.touch(r);this.events.emit(r.id);});
 ...
 finish(r,reason){
  ...
  const {child,secret,idleTimer,killTimer,waiting,status,...saved}=r;this.data.sessions.push({...saved});this.data.sessions=this.data.sessions.slice(-this.historyLimit);this.persist();
  this.events.emit(r.id);
 }
 ...
 shutdown(){this.closed=true;for(const r of [...this.live.values()]){r.endReason='server';try{r.child.kill('SIGKILL');}catch{}this.finish(r,'server');}}
}
