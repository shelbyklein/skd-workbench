...
import {TranscriptReader,findClaudeTranscript,findClaudeTranscriptByFolder,findCodexTranscript} from './transcripts.js';
...
export function projectSessionArgs(provider,{model,effort,system,prompt,server,cwd,full=false,sessionID=null}){
 ...
 if(provider==='claude')return ['--mcp-config',JSON.stringify({mcpServers:{workbench:{type:'stdio',command:server.command,args:server.args}}}),'--append-system-prompt',system,
  ...
  ...(sessionID?['--session-id',sessionID]:[]),...(full?['--dangerously-skip-permissions']:[]),'--allowedTools',...projectFreeTools.map(t=>'mcp__workbench__'+t),'--model',model,...(effort==='default'?[]:['--effort',effort]),...(prompt?['--',prompt]:[])];
...
export function sessionArgs(provider,{model,effort,system,prompt,server,workspace,full=false,sessionID=null}){
 const hookCommand=`${toml(server.command)} ${toml(hook)} waiting`;
 if(provider==='claude'){
  ...
  return ['--append-system-prompt',system,'--mcp-config',JSON.stringify({mcpServers:{workbench:{type:'stdio',command:server.command,args:server.args}}}),
   '--settings',JSON.stringify(settings),'--no-chrome',...(sessionID?['--session-id',sessionID]:[]),...(full?['--dangerously-skip-permissions']:['--permission-mode','manual']),'--tools','Skill',
...
export class CoordinatorSessions{
 constructor(directory,{spawn=pty.spawn,nodePath=process.execPath,idleMs=1800000,historyLimit=20,tailChars=262144,killDelay=3000,sessionHost=false,host=null,transcriptHome=undefined}={}){
  this.transcriptHome=transcriptHome;
 ...
 start(threadKey,{provider,model,effort,binary,credentialPath,system,prompt,env={},cwd=null,project=null,full=false}){
  ...
  // When Workbench itself runs inside a Claude Code session, these would tie the agent to that session (and its
  // transcript) instead of its own.
  for(const key of ['CLAUDECODE','CLAUDE_CODE_SESSION_ID','CLAUDE_CODE_CHILD_SESSION','CLAUDE_CODE_ENTRYPOINT','CLAUDE_CODE_HOST_SESSION_ID','CLAUDE_CODE_MESSAGING_SOCKET','CLAUDE_CODE_MESSAGING_TOKEN','CLAUDE_CODE_SESSION_ATTENDED','CLAUDE_CODE_SDK_HAS_HOST_AUTH_REFRESH','CLAUDE_AGENT_SDK_VERSION','CLAUDE_CODE_EXECPATH','CLAUDE_PID'])delete childEnv[key];
  // Claude records the session under this ID, so Chat can follow its transcript (#48).
  const args=cwd?projectSessionArgs(provider,{model,effort,system,prompt,server,cwd,full,sessionID:id}):sessionArgs(provider,{model,effort,system,prompt,server,workspace:this.workspace,full,sessionID:id});
 ...
 // Chat events for a conversation from the CLI transcripts of its live session and a few previous ones (#48).
 async transcript(threadKey,{previous=3}={}){
  this.readers??=new Map();
  const live=this.live.get(threadKey),sessions=[...this.data.sessions.filter(s=>s.threadKey===threadKey).slice(-previous),...(live?[live]:[])];
  const claimed=new Set([...this.data.sessions,...this.live.values()].map(s=>s.transcriptFile).filter(Boolean)),events=[];let working=false;
  for(const s of sessions){
   if(!s.transcriptFile){
    const cwd=s.cwd||this.workspace;
    const home=this.transcriptHome;
    s.transcriptFile=s.provider==='claude'?(await findClaudeTranscript(s.id,{home})||await findClaudeTranscriptByFolder({cwd,startedAt:s.startedAt,claimed,home})):await findCodexTranscript({cwd,startedAt:s.startedAt,claimed,home});
    if(!s.transcriptFile)continue;claimed.add(s.transcriptFile);if(s.status!=='running')this.persist();
   }
   // A Claude session can continue in a new transcript file; follow the chain.
   let file=s.transcriptFile;
   for(let hop=0;file&&hop<5;hop++){
    let reader=this.readers.get(file);if(!reader){reader=new TranscriptReader(file,s.provider);this.readers.set(file,reader);}
    const view=await reader.read();events.push(...view.events.map(e=>({...e,sessionID:s.id})));if(s===live&&view.working)working=true;
    file=view.continuedIn?path.join(path.dirname(file),view.continuedIn+'.jsonl'):null;
   }
  }
  // A continued Claude transcript repeats the earlier history; keep one copy of each event, the latest state of it.
  const unique=new Map();for(const e of events)unique.set(e.id,unique.has(e.id)?{...unique.get(e.id),...e}:e);events.splice(0,events.length,...unique.values());
  // Busy: a Codex turn is open, or the terminal printed something in the last few seconds.
  const busy=!!live&&(working||Date.now()-(live.lastOutputAt||0)<4000);
  return {threadKey,sessionID:live?.id||null,provider:live?.provider||null,running:live?.status==='running',waiting:!!live?.waiting,busy,events:events.slice(-400)};
 }
 // Answers from Chat, typed into the CLI the way the person would (#48). A Claude question takes the number of
 // each chosen option, then Enter; a Claude permission prompt takes 1 (allow once) or Esc (deny).
 answer(id,input){
  const r=[...this.live.values()].find(x=>x.id===id);assert(r&&r.status==='running','This session is not running.',409);
  assert(input&&typeof input==='object','Choose an answer.');let keys;
  if(Array.isArray(input.options)){assert(r.provider==='claude'&&input.options.length>=1&&input.options.length<=4&&input.options.every(i=>Number.isInteger(i)&&i>=0&&i<=8),'Choose one option for each question.');keys=[...input.options.map(i=>String(i+1)),'\r'];}
  else if(['allow','deny'].includes(input.permission)){assert(r.provider==='claude','Answer this prompt in the CLI.');keys=[input.permission==='allow'?'1':'\x1b'];}
  else assert(false,'Choose an answer.');
  r.waiting=false;this.touch(r);
  const wait=ms=>new Promise(resolve=>{const t=setTimeout(resolve,ms);t.unref?.();});
  r.chain=(r.chain||Promise.resolve()).then(async()=>{for(const key of keys){if(r.status!=='running')return;r.child.write(key);await wait(400);}}).catch(()=>{});
  this.events.emit(r.id);return this.summary(r);
 }
