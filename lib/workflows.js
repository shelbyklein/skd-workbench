...
export class Workflows{
 ...
 async contexts(input,flow,project,{preview=false}={}){
  ...
  for(const step of flow.steps.filter(s=>s.type==='agent')){
   ...
   assert(!c.agentProfileID&&!c.playbook&&!c.agent,'Workflow steps use Codex and an agentProfile assignment.');
   ...
   const profile=profiles?await profiles[preview?'preview':'resolveLaunch'](project,'codex',requested,{purpose:'workflow',mode:input.mode}):null;
   ...
   const skillContext=profile?.skills||this.codex.skills?.resolve(project,'codex',skills,{purpose:'workflow'})||{status:'empty',entries:[],text:''};
   contexts[step.id]={agentProfile:profile,skills:skillContext,connections:profile?.connections||{status:'excluded',connections:[],reason:'Structured workflows do not use MCP connections.'}};
 ...
 async start(input,flow,project){
  ...
  try{
   ...
   for(const s of flow.steps.filter(s=>s.type==='agent')){const c=input.config?.[s.id],m=provider.models.find(m=>m.id===c?.model);assert(m&&m.efforts.includes(c.effort),`Choose a supported Codex model and effort for ${s.name}.`);const skills=c.skills||s.skills||{mode:'inherit',skillIDs:[]};assert(['inherit','replace'].includes(skills.mode)&&Array.isArray(skills.skillIDs),`Choose valid skills for ${s.name}.`);config[s.id]={model:c.model,effort:c.effort,skills:{mode:skills.mode,skillIDs:[...new Set(skills.skillIDs)]}};}
