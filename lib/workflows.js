...
import {assert,copy,id,now,validateFlow,issueDirection} from './domain.js';
...
export class Workflows{
 ...
 async start(input,flow,project){
  assert(!input.agentProfile&&!input.playbook&&!flow.steps.some(s=>s.agentProfile||s.agentProfileID||s.playbook)&&!Object.values(input.config||{}).some(c=>c.agentProfile||c.agentProfileID||c.playbook),'Agent profiles for workflow steps are not supported yet.');
  ...
  try{
   ...
   const r={id:runID,revision:0,flow:copy(flow),issueSnapshots,projectID:project.id,projectSnapshot:copy(project),sourceContext:context,task,acceptance,mode:input.mode,config,maxAttempts:input.maxAttempts,cursor:0,status:'launching',attempts:[],retries:{},workspace:null,createdAt:now(),finishedAt:null,error:null};
 ...
 async advance(runID){
  ...
  try{
   const previous=r.attempts.find(a=>a.id!==attempt.id&&a.stepID===step.id&&a.childID),agentContext=previous?.childID?this.codex.get(previous.childID).agentContext:null;
   const child=await this.codex.start({task:prompt,mode:r.mode,...r.config[step.id]},r.projectSnapshot,{workflowID:r.id,workspacePurpose:r.task,workRef:'local:'+r.id,attemptID:attempt.id,workspace:r.workspace,expectedContext:r.sourceContext,agentContext,shouldLaunch:()=>!this.closed&&this.raw(r.id).status==='launching'});
