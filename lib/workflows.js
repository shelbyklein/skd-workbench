...
export class Workflows{
 ...
 async start(input,flow,project){
  assert(!this.codex.owner&&!this.codex.active&&!this.codex.starting,'Finish or stop the active Codex task/workflow first.',409);
 ...
 async advance(runID){
  ...
  if(!step){this.change(runID,r=>{r.status='completed';r.finishedAt=now();});this.codex.owner=null;return;}
 ...
 finished(child){
  ...
  if(terminal(r))this.codex.owner=null;else if(r.status==='launching')this.queue(r.id);
 }
 action(runID,input){
  ...
  if(input.action==='stop'){
   ...
   if(r.status==='stopping')this.codex.stop(this.codex.active.id,r.id);else this.codex.owner=null;return this.get(runID);
  }
