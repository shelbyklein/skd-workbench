...
export class WorktreeNotes {
  ...
  change(fn){const draft=copy(this.data),result=fn(draft);this.validate(draft);this.persist(draft);this.data=draft;if(this.onRecord)this.onRecord(copy(result));return copy(result);}
  ...
  prepare(input){
    ...
    if(existing){assert(same(existing.preparation,preparation),'Registration intent already belongs to different creation input.',409);if(this.onRecord)this.onRecord(copy(existing));return copy(existing);}
  ...
  async restore(snapshot,{projectID,validate=()=>{}}){
    const restored=copy(snapshot);this.validate({version:1,records:[restored]});assert(restored.state==='attached'&&restored.attachment,'Only previously verified attachments can be restored.',409);
    const existing=this.data.records.find(r=>r.id===restored.id);if(existing){assert(existing.projectID===projectID&&same(existing.attachment,restored.attachment),'Restored registration conflicts with local identity.',409);validate();return this.verify(existing.id,existing.destination,existing.ownerKey);}
    assert(!this.data.records.some(r=>r.intentID===restored.intentID||r.destination===restored.destination),'Workspace already has another registration.',409);
    const actual=await inspect(restored);assert(same(actual,restored.attachment),'Restored filesystem identity does not match. Keep this record unattached.',409);validate();
    restored.restoredFrom={projectID:restored.projectID,restoredAt:now()};restored.projectID=projectID;restored.preparation.projectID=projectID;
    return this.change(d=>{assert(!d.records.some(r=>r.id===restored.id||r.intentID===restored.intentID||r.destination===restored.destination),'Registration changed during restoration.',409);d.records.push(restored);return restored;});
  }
