...
export class CoordinatorAgent{
 ...
 view(){
  ...
  // Live project agents, for the side panel's split view on Home.
  const projects=new Map(this.projects().map(p=>[p.id,p]));
  const running=[...this.sessions.live.values()].filter(r=>r.threadKey!==COORDINATOR&&r.status==='running'&&projects.has(r.threadKey)).map(r=>{const p=projects.get(r.threadKey);return {projectID:p.id,name:p.name,folderPath:p.folderPath,waiting:!!r.waiting,provider:r.provider,model:r.model};});
