...
function validate(value){
 ...
 value.projectAgents??={};
 // Full permissions (no CLI permission prompts) was added later; it is off unless the person turns it on.
 assert(value.fullPermissions===undefined||typeof value.fullPermissions==='boolean',damaged);value.fullPermissions??=false;
 // Project agents the person stopped, with the orchestrator messages held for them (added later).
 assert(value.stoppedAgents===undefined||value.stoppedAgents&&typeof value.stoppedAgents==='object'&&!Array.isArray(value.stoppedAgents)&&Object.values(value.stoppedAgents).every(s=>s&&Array.isArray(s.pending)),damaged);value.stoppedAgents??={};
 return value;
}
...
- Hand the request to that project's agent with message_project_agent, passing the full relevant text and a new unique requestKey. Then tell the user briefly which project it went to.
- Messages that start "[from <Project> agent · question|action|update]" are reports from a project agent. They already appear in the user's chat, attributed to that agent, so do not repeat them. Reply only if you add something the user needs (for example, which earlier request it concerns). When the user answers, forward the answer to that project with message_project_agent.
- When the user reports a bug or asks for a change in SKD Workbench itself (this app, not one of their projects), file it with create_workbench_issue: a clear title and a body with what happened, what they expected and any steps, written from the user's words. Then give the user the issue link.
...
- Answer the user directly in your reply. The Workbench chat shows your replies, your tool steps and your questions as you write them, so you do not need to post them. If message_project_agent returns held:true, tell the user that agent is stopped and waits for them.
Answer concisely.`;
...
Rules:
- The user reads the Workbench chat, which shows your replies, tool steps and questions as you write them. Answer directly in your reply; you do not need to post it.
...
export class CoordinatorAgent{
 constructor(directory,{threads,controllers,projects,executor,sessions,sessionOptions={}}){
  ...
  }else this.data={schema:2,version:0,enabled:false,provider:'claude',model:null,effort:'default',legacyTurns:[],projectAgents:{},fullPermissions:false,stoppedAgents:{}};
 }
 ...
 view(){
  ...
  // Project agents for the side panel's pills on Home: on, working (output in the last few seconds), needs you,
  // or off (its session ended in the last 12 hours).
  const projects=new Map(this.projects().filter(p=>p.folderPath).map(p=>[p.id,p])),now=Date.now(),running=[];
  for(const r of this.sessions.live.values()){if(r.threadKey===COORDINATOR||r.status!=='running'||!projects.has(r.threadKey))continue;const p=projects.get(r.threadKey);
   running.push({projectID:p.id,name:p.name,folderPath:p.folderPath,waiting:!!r.waiting,state:r.waiting?'waiting':now-(r.lastOutputAt||0)<4000?'working':'on',provider:r.provider,model:r.model});}
  const ended=new Map();for(const s of this.sessions.data.sessions)if(projects.has(s.threadKey)&&!running.some(r=>r.projectID===s.threadKey)&&now-Date.parse(s.endedAt||0)<12*3600000)ended.set(s.threadKey,s);
  for(const [id,s] of ended){const p=projects.get(id);running.push({projectID:id,name:p.name,folderPath:p.folderPath,waiting:false,state:'off',endReason:s.endReason||null,provider:s.provider,model:s.model});}
  running.sort((a,b)=>a.name.localeCompare(b.name));
  return {...copy(settings),grant:g?{id:g.id,projectIDs:[...g.projectIDs],capabilities:[...g.capabilities]}:null,freeTools,promptTools,waiting:this.sessions.waiting(),running};
 }
 state(threadKey){const v=this.view();return {enabled:v.enabled,provider:v.provider,model:v.model,effort:v.effort,grant:!!v.grant,session:this.sessions.current(threadKey),history:this.sessions.history(threadKey),...(threadKey===COORDINATOR?{}:{agent:this.projectAgent(threadKey)})};}
 async save(input,endpoint){
  assert(input&&Object.keys(input).every(k=>['version','enabled','provider','model','effort','fullPermissions'].includes(k)),'Unknown coordinator setting.');
  assert(input.fullPermissions===undefined||typeof input.fullPermissions==='boolean','Choose whether agents run with full permissions.');
  ...
  // Full permissions applies to sessions started after saving; running sessions keep the prompts they started with.
  Object.assign(this.data,{enabled:input.enabled,provider:input.provider,model:input.model,effort:input.effort,fullPermissions:input.fullPermissions??this.data.fullPermissions,version:this.data.version+1});this.persist();
 ...
 onMessage(threadKey,message,scope){
  ...
  return this.sessions.deliver(threadKey,message.text,()=>this.launch(threadKey,scope,'answer the last user message'));
 }
 ...
 launch(threadKey,scope,task){
  ...
  return {provider:this.data.provider,model:this.data.model,effort:this.data.effort,binary:this.data.provider==='claude'?this.executor.claudeBinary:this.executor.binary,credentialPath,full:this.data.fullPermissions,system:instructions(scope),
   prompt:`CONVERSATION SO FAR (oldest first; ${task}):\n${transcript(messages)||'(no messages yet)'}`};
 }
 // Stop from the CLI view. A project agent the person stops stays stopped: the orchestrator cannot start it again,
 // and its messages wait until the person starts it (Start session or their own message).
 stop(sessionID){
  const session=this.sessions.stop(sessionID);
  if(session.threadKey!==COORDINATOR){this.data.stoppedAgents[session.threadKey]??={at:new Date().toISOString(),pending:[]};this.persist();}
  return session;
 }
 ...
 async startProject(project,prompt){
  ...
  if(this.endpoint)this.controllers.refreshEndpoint(grant.id,this.endpoint);
  const credentialPath=this.controllers.list().find(c=>c.id===grant.id).credentialPath;
  // Only the person starts a stopped agent (every caller here is theirs). Held orchestrator messages go into the
  // first prompt, after the person's own message, so none is typed before the CLI is ready.
  const held=this.data.stoppedAgents[project.id]?.pending||[];if(this.data.stoppedAgents[project.id]){delete this.data.stoppedAgents[project.id];this.persist();}
  const session=this.sessions.start(project.id,{provider:agent.provider,model:agent.model,effort:agent.effort,binary:agent.provider==='claude'?this.executor.claudeBinary:this.executor.binary,credentialPath,full:this.data.fullPermissions,cwd:place.cwd,project:project.id,
   system:projectInstructions(project,place.cwd,place.branch),prompt:[prompt,...held.map(text=>`[from orchestrator] ${text}`)].filter(Boolean).join('\n\n')});
  return session;
 }
 ...
 async messageProjectAgent(project,text,requestKey,controller){
  ...
  const stopped=this.data.stoppedAgents[project.id];
  if(stopped&&!this.sessions.isLive(project.id)){
   if(!stopped.pending.includes(text)){stopped.pending.push(text);stopped.pending=stopped.pending.slice(-20);this.persist();}
   return {projectID:project.id,messageID:message.id,session:null,held:true,note:'The user stopped this project agent. Your message is saved in its conversation and will be delivered when the user starts the agent again. Do not retry or start it; tell the user it is waiting for them.'};
  }
 ...
 // Agent questions and action requests stay open until the person sends any message afterwards, to that
 // project's agent or to the orchestrator. Updates never need attention. Only recent history is scanned.
 openQuestions(projectID){
  const coordinator=this.threads.recent(COORDINATOR,200).items,lastUser=items=>items.filter(m=>m.author==='user').map(m=>m.createdAt).sort().at(-1)||'';
  const repliedCoordinator=lastUser(coordinator),open=[];
  for(const m of coordinator){
   if(m.author!=='agent'||!String(m.requestKey||'').startsWith('report:'))continue;
   const kind=m.text.startsWith('Question: ')?'question':m.text.startsWith('Action needed: ')?'action':null;if(!kind)continue;
   const pid=m.refs.find(r=>r.kind==='project')?.id;if(!pid||(projectID&&pid!==projectID))continue;
   const replied=[repliedCoordinator,lastUser(this.threads.recent(pid,100).items)].some(at=>at>m.createdAt);if(replied)continue;
   const project=this.projects().find(p=>p.id===pid);if(!project)continue;
   open.push({kind:'question',messageID:m.id,projectID:pid,projectName:project.name,title:kind==='question'?'Agent asks':'Agent needs an action',detail:m.text.replace(/^(Question|Action needed): /,'').slice(0,240),createdAt:m.createdAt});
  }
  return open;
 }
 // A project's running timeline, newest first: its agent's reports (updates, questions, action requests) from the
 // orchestrator chat, plus the project conversation (your messages and the orchestrator's requests). With an issue
 // number, only entries that mention it (#N) are kept. Built from stored messages; nothing new is tracked.
 projectTimeline(projectID,{issue=null,limit=60}={}){
  const entries=[],mention=issue?new RegExp(`(^|[^\\w])#${Number(issue)}(?!\\d)`):null;
  const replies=[...this.threads.recent(COORDINATOR,200).items,...this.threads.recent(projectID,200).items].filter(m=>m.author==='user').map(m=>m.createdAt).sort();
  for(const m of this.threads.recent(COORDINATOR,200).items){
   if(m.author!=='agent'||!String(m.requestKey||'').startsWith('report:'+projectID+':'))continue;
   const kind=m.text.startsWith('Question: ')?'question':m.text.startsWith('Action needed: ')?'action':'update';
   entries.push({id:m.id,at:m.createdAt,kind,who:m.controllerName||'Project agent',text:m.text.replace(/^(Question|Action needed|Update): /,''),...(kind==='update'?{}:{answered:replies.some(at=>at>m.createdAt)})});
  }
  for(const m of this.threads.recent(projectID,200).items){
   const fromOrchestrator=m.author==='agent'&&m.text.startsWith('From the orchestrator: ');
   entries.push({id:m.id,at:m.createdAt,kind:m.author==='user'?'you':fromOrchestrator?'request':'agent',who:m.author==='user'?'You':fromOrchestrator?'Orchestrator':(m.controllerName||'Project agent'),text:fromOrchestrator?m.text.slice(23):m.text});
  }
  return entries.filter(e=>!mention||mention.test(e.text)).sort((a,b)=>b.at.localeCompare(a.at)).slice(0,limit);
 }
