...
export class CoordinatorAgent{
 constructor(directory,{threads,controllers,projects,executor,sessions,sessionOptions={}}){
  ...
  }else this.data={schema:2,version:0,enabled:false,provider:'claude',model:null,effort:'default',legacyTurns:[],projectAgents:{},fullPermissions:false};
 }
 ...
 async startProject(project,prompt){
  ...
  if(this.endpoint)this.controllers.refreshEndpoint(grant.id,this.endpoint);
  const credentialPath=this.controllers.list().find(c=>c.id===grant.id).credentialPath;
  return this.sessions.start(project.id,{provider:agent.provider,model:agent.model,effort:agent.effort,binary:agent.provider==='claude'?this.executor.claudeBinary:this.executor.binary,credentialPath,full:this.data.fullPermissions,cwd:place.cwd,project:project.id,
