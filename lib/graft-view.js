...
async function discoverIndex(project){
 ...
 for(const candidateRoot of roots){
  ...
  let graftDir;
  try{graftDir=await realpath(candidate);}catch{continue;}
  ...
  if(hasGraph||hasContext)return {available:true,folder,repository,graftDir,indexScope:candidateRoot===folder?'project':'repository'};
 }
