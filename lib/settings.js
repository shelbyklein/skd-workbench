...
export class Settings {
 constructor(directory){
  ...
  this.data=original!==null?this.validate(JSON.parse(original)):{version:1,accents:{light:'#bf502f',dark:'#ed9777'},hiddenModels:[],projectTags:[]};
 ...
 validate(value){
  ...
  return {projectTags,version:value.version,accents:{...value.accents},hiddenModels:[...new Set(value.hiddenModels)]};
 }
 save(value,projects){
  const next=this.validate({...value,projectTags:value.projectTags===undefined?this.data.projectTags:Array.isArray(value.projectTags)?value.projectTags.map(tag=>tag&&tag.color===undefined?{...tag,color:this.data.projectTags.find(t=>t.id===tag.id)?.color}:tag):value.projectTags});
