...
export class Settings {
 constructor(directory){
  ...
  this.data=original!==null?this.validate(JSON.parse(original)):{version:1,primaryColor:'#315b74',accents:{light:'#bf502f',dark:'#ed9777'},hiddenModels:[],projectTags:[],hiddenProjects:[]};
 ...
 validate(value){
  ...
  // Projects hidden from Home (still reachable from their pages and the side panel).
  const hiddenProjects=value.hiddenProjects===undefined?[]:value.hiddenProjects;
  assert(Array.isArray(hiddenProjects)&&hiddenProjects.length<=10000&&hiddenProjects.every(id=>typeof id==='string'&&/^[\w-]{1,100}$/.test(id)&&id!=='unassigned'),'Invalid hidden projects.');
  return {projectTags,hiddenProjects:[...new Set(hiddenProjects)],primaryColor,version:value.version,accents:{...value.accents},hiddenModels:[...new Set(value.hiddenModels)]};
 }
 save(value,projects){
  const next=this.validate({...value,hiddenProjects:value.hiddenProjects===undefined?this.data.hiddenProjects:value.hiddenProjects,primaryColor:value.primaryColor===undefined?this.data.primaryColor:value.primaryColor,projectTags:value.projectTags===undefined?this.data.projectTags:Array.isArray(value.projectTags)?value.projectTags.map(tag=>tag&&tag.color===undefined?{...tag,color:this.data.projectTags.find(t=>t.id===tag.id)?.color}:tag):value.projectTags});
  if(projects)next.hiddenProjects=next.hiddenProjects.filter(id=>projects.some(p=>p.id===id));
