...
export async function inspectRepository(folder,{targetRef='',remoteName='',run=readGit,limit=100,worktreeLimit=30}={}){
  ...
  const summary={branches:allBranches.length,worktrees:rawTrees.length,checkedOutBranches:new Set(worktrees.filter(w=>w.branch).map(w=>w.branch)).size,
    ...
    dirtyWorktrees:worktrees.filter(w=>w.dirty===true).length,conflictedWorktrees:worktrees.filter(w=>w.changes?.conflicts>0).length,
    operationWorktrees:worktrees.filter(w=>w.operations?.length).length,detachedWithCommits:target?worktrees.filter(w=>w.detached&&w.comparison?.ahead>0).length:null};
