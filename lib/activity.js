...
function owner(activity,toolName,server,connections=[]){
 const explicit=connections.find(item=>server&&(item.name===server||item.id===server));if(explicit)return {connectionID:explicit.id,connectionName:explicit.name};
 const match=/^mcp__([^_]+(?:_[^_]+)*)__(.+)$/.exec(toolName||''),mapped=match&&connections.find(item=>item.name===match[1]);return mapped?{connectionID:mapped.id,connectionName:mapped.name}:{connectionID:null,connectionName:server||null};
}
