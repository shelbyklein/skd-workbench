...
export class GitHubIssues{
 ...
 async count(project){
  ...
  try{
   ...
   // GraphQL has its own rate limit; the REST search count keeps Home's totals when only GraphQL is refused.
   let raw;try{raw=await this.request(`search/issues?q=${encodeURIComponent(`repo:${repository} is:issue is:open`)}&per_page=1`);}catch{throw error;}
   if(!Number.isSafeInteger(raw?.total_count)||raw.total_count<0)throw error;
   return {repository,state:'open',count:raw.total_count,checkedAt:now()};
  }
 }
...
// Issues about SKD Workbench itself, filed by the coordinator on the repository of the Workbench checkout.
// Each requestKey files at most once: a retry returns the issue already filed.
export class WorkbenchIssues{
 constructor(directory,{github,folder,repository}={}){
  mkdirSync(directory,{recursive:true});this.file=path.join(directory,'workbench-issues.json');this.github=github;this.pending=new Map();
  this.repository=repository||(async()=>repositoryFrom(await inspectFolder(folder)));
  this.records=existsSync(this.file)?JSON.parse(readFileSync(this.file,'utf8')):[];
 }
 persist(){writeFileSync(this.file+'.tmp',JSON.stringify(this.records.slice(-500),null,2),{mode:0o600});renameSync(this.file+'.tmp',this.file);}
 create({requestKey,title,body}){
  const done=this.records.find(r=>r.requestKey===requestKey);if(done)return Promise.resolve({...done,duplicate:true});
  if(this.pending.has(requestKey))return this.pending.get(requestKey);
  const job=(async()=>{
   const repository=await this.repository();
   const issue=await this.github.request(`repos/${repository}/issues`,'POST',{title:title.trim(),body:`${body.trim()}\n\n_Filed by the SKD Workbench coordinator._`});
   const record={requestKey,repository,number:issue.number,url:issue.html_url,title:issue.title,createdAt:now()};
   this.records.push(record);this.persist();return record;
  })().finally(()=>this.pending.delete(requestKey));
  this.pending.set(requestKey,job);return job;
 }
}
