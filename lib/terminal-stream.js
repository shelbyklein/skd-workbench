...
export function attachTerminalStreams(server,{workspace,agents,ackTimeout=15000}){
 const wss=new WebSocketServer({noServer:true,maxPayload:70000,perMessageDeflate:false});
 function upgrade(req,socket,head){
  let manager,id,cursor;
  try{
   const host=req.headers.host||'';
   assert(/^(127\.0\.0\.1|localhost):\d+$/.test(host)&&req.headers.origin===`http://${host}`,'Local origin required.',403);
   assert(wss.clients.size<32,'Too many terminal connections.',429);
   const url=new URL(req.url,`http://${host}`),match=url.pathname.match(/^\/api\/(workspace-terminal|terminal-sessions)\/([\w-]+)\/stream$/);
   assert(match,'Unknown terminal stream.',404);[, ,id]=match;manager=match[1]==='workspace-terminal'?workspace:agents;
