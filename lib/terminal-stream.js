...
export function attachTerminalStreams(server,{agents,coordinator,remoteAccess,ackTimeout=15000}){
 ...
 async function upgrade(req,socket,head){
  ...
  try{
   ...
   const url=new URL(req.url,`http://${host}`),match=url.pathname.match(/^\/api\/(terminal-sessions|coordinator-terminal)\/([\w-]+)\/stream$/);
   assert(match,'Unknown terminal stream.',404);[, ,id]=match;manager=match[1]==='coordinator-terminal'?coordinator:agents;assert(manager,'Unknown terminal stream.',404);
