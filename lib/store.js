...
export class Store {
  ...
  saveRunSettings(flowID,input){return this.change(d=>{
    ...
    const steps=f.steps.map(s=>s.type==='agent'&&input.config?.[s.id]?{...s,model:input.config[s.id].model,effort:input.config[s.id].effort,skills:input.config[s.id].skills||s.skills||{mode:'inherit',skillIDs:[]}}:s);
