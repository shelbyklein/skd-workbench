...
export async function toolsInventory({connections,projects,project=null,home=homedir()}){
 ...
 // Claude.ai connectors belong to the account; Claude Code records the ones it has connected in ~/.claude.json.
 // Their live status would need `claude mcp list`, which starts every configured server, so none is shown.
 const claudeState=await readJSON(path.join(home,'.claude.json')),known=new Set(result.claude.mcp.map(m=>m.name));
 for(const name of Array.isArray(claudeState?.claudeAiMcpEverConnected)?claudeState.claudeAiMcpEverConnected.slice(0,100):[])
  if(typeof name==='string'&&name.startsWith('claude.ai ')&&!known.has(name))result.claude.mcp.push({name:name.slice(10,110),source:'account',sourcePath:'claude.ai',transport:'connector',enabled:null,shadowed:false,inlineCredentials:false,environmentNames:[]});
