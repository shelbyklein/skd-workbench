...
export function createWorkspaceTerminal(folder,{spawn=pty.spawn,shell=process.platform==='darwin'?'/bin/zsh':'/bin/bash'}={}){
 const cwd=realpathSync(folder);let current=null;const events=new EventEmitter();events.setMaxListeners(32);
 ...
 return {
  ...
  subscribe(id,listener){record(id);events.on(id,listener);return ()=>events.off(id,listener);},
  open(){
   ...
   child.onData(data=>{r.output+=data;if(r.output.length>200000){const trim=r.output.length-200000;r.output=r.output.slice(trim);r.offset+=trim;}events.emit(r.id);});
   child.onExit(()=>{if(r.status==='running')r.status='completed';events.emit(r.id);});return summary();
  },
  ...
  stop(id){const r=record(id);if(r.status==='running'){r.status='stopped';r.child.kill();events.emit(r.id);}return summary();},
