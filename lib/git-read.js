...
export async function readGit(folder,args,{raw=false,timeout=6000,network=false}={}){
  ...
  try{
    ...
    if(network){networkPID=pending.child.pid;timer=setTimeout(()=>{try{process.kill(-networkPID,'SIGKILL');}catch{}},timeout);}
  ...
  finally{clearTimeout(timer);if(networkPID)try{process.kill(-networkPID,'SIGKILL');}catch{}}
}
