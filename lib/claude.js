import {deliveredInstructions} from './agent-context.js';
import {execFile,spawn} from 'node:child_process';
import {promisify} from 'node:util';
import {existsSync} from 'node:fs';
import {homedir} from 'node:os';
import path from 'node:path';
import {Problem,toolFreePurposes} from './domain.js';
...
export async function discoverClaude(binary=claudeBinary(),{allowSignedOut=false}={}){
 const [{stdout:version},{stdout:help},auth]=await Promise.all([exec(binary,['--version'],{timeout:10000}),exec(binary,['--help'],{timeout:10000}),exec(binary,['auth','status','--json'],{timeout:15000}).catch(e=>({stdout:e.stdout||'',error:e}))]);
 ...
 if(auth.error?.killed)throw new Problem('Claude authentication check timed out. Retry when the CLI responds; sign-in status is unknown.',503);
...
export function claudeArgs(r){
 const tools=toolFreePurposes.includes(r.purpose)?'':r.mode==='worktree'?'Read,Glob,Grep,Edit,Write':'Read,Glob,Grep';
 return ['--print','--output-format','stream-json','--verbose','--safe-mode','--restricted','--strict-mcp-config','--mcp-config','{"mcpServers":{}}',...(deliveredInstructions(r.agentContext)?['--append-system-prompt',deliveredInstructions(r.agentContext)]:[]),'--settings','{"disableAllHooks":true}','--disable-slash-commands','--no-chrome','--no-session-persistence','--permission-mode','dontAsk','--tools',tools,'--allowedTools',tools,'--model',r.model,...(r.effort==='default'?[]:['--effort',r.effort])];
}
