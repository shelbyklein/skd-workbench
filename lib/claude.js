...
export async function discoverClaude(binary=claudeBinary()){
 ...
 if(!status.loggedIn)throw new Problem('Claude is not signed in. Run claude auth login in Terminal, then retry.',503);
 return {available:true,version:version.trim(),auth:'Signed in',modelNote:'Claude CLI aliases; availability is confirmed when execution succeeds.',models:['sonnet','opus','fable','haiku'].map(id=>({id,name:id[0].toUpperCase()+id.slice(1),efforts:['low','medium','high'],defaultEffort:'medium',isDefault:id==='sonnet'})),checkedAt:new Date().toISOString()};
}
export function claudeArgs(r){
 const tools=r.mode==='worktree'?'Read,Glob,Grep,Edit,Write':'Read,Glob,Grep';
