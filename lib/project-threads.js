...
export const refKinds=['run','issue','workflow','session','operation','project'];
// The coordinator thread spans projects; its key cannot collide with a project ID.
...
export class ProjectThreads{
 constructor(directory,{runs=()=>[],projects=()=>[]}={}){
  mkdirSync(directory,{recursive:true});this.file=path.join(directory,'project-threads.json');this.runs=runs;this.projects=projects;
 ...
 refs(projectID,value){
  ...
  return (value||[]).map(r=>{
   ...
   if(r.kind==='run')assert(this.runs().some(run=>run.id===r.id&&(projectID===COORDINATOR||run.projectID===projectID)),'Linked run is not in this project.',404);
   if(r.kind==='project')assert(this.projects().some(p=>p.id===r.id&&p.id!=='unassigned'),'Linked project not found.',404);
 ...
 post(project,{author,text,refs,requestKey,controller=null,source=null}){
  assert(project.id===COORDINATOR||project.id!=='unassigned'&&project.folderPath,'Connect a project folder before starting a conversation.');
  ...
  const links=this.refs(project.id,refs),body={author,text:text.trim(),refs:links,...(source==='cli'?{source}:{})},fp=fingerprint(body),owner=controller?.id||null;
