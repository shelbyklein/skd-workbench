...
export class Skills {
 ...
 resolve(project,provider,selection={mode:'inherit',skillIDs:[]},{purpose='session'}={}){
  ...
  if(toolFreePurposes.includes(purpose))return {status:'excluded',reason:purpose==='issue-proposal'?'Issue edit proposals do not receive managed skills.':'Briefings do not receive managed skills.',policyRevision:this.data.revision,entries:[],text:''};
  // Sessions use each CLI's own skills (user, plugin and the project's repo skills); Workbench no longer injects a
  // managed selection. The library and policies stay on disk and readable for history (#20).
  return {status:'native',reason:'Each CLI uses its own skills.',policyRevision:this.data.revision,entries:[],text:''};
 }
 resolveManaged(project,provider,selection={mode:'inherit',skillIDs:[]}){
