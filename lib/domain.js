...
// Internal report runs: no tools, shell, web, MCP, skills or GitHub credentials.
export const toolFreePurposes=['issue-proposal','daily-briefing'];
...
export function validateFlow(input) {
  ...
  const steps = input.steps.map((s, index) => {
    ...
    if (s.type === 'agent') {
      assert(['codex','claude'].includes(s.agent??'codex'),'Choose Codex or Claude for this step.');
      if(s.agent!==undefined)result.agent=s.agent;
      result.model = text(s.model, 'Model', 150);
      assert(['none','minimal','low','medium','high','xhigh','max',...((s.agent||'codex')==='claude'?['default']:[])].includes(s.effort), 'Choose a valid effort.');
