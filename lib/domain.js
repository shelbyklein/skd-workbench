...
export function validateFlow(input) {
  ...
  const steps = input.steps.map((s, index) => {
    assert(s && ['agent', 'human', 'check'].includes(s.type), 'Choose a valid step type.');
    ...
    if (s.type === 'agent') {
      ...
      assert(['low','medium','high','max'].includes(s.effort), 'Choose a valid effort.');
...
export function newStep(type, name, model = '') {
  return {id:id(), type, name, instructions:'', ...(type === 'agent' ? {model, effort:'high'} : type === 'human' ? {maxRetries:0, retryFrom:null} : {})};
}
...
export function createRun(flow, input, project=null, sourceContext=null) {
  ...
  const task=text(input.task,'Task',12000),acceptance=text(input.acceptance||'','Acceptance checks',12000,false);
  return {id:id(),revision:0,mode:'simulation',flow:copy(flow),task,acceptance,
    projectID:project?.id||'unassigned',projectSnapshot:copy(project),sourceContext:copy(sourceContext),
    comparisonKey:createHash('sha256').update(JSON.stringify([task,acceptance,project?.id||'unassigned',sourceContext?.folderPath||null,sourceContext?.git?.root||null,sourceContext?.git?.commit||null,sourceContext?.git?.dirty??null])).digest('hex'),
