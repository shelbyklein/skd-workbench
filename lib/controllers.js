...
export class Controllers{
 constructor(directory,{projects}){
  ...
  assert(this.data.controllers.every(c=>typeof c.id==='string'&&/^[a-f0-9]{64}$/.test(c.tokenHash)&&Array.isArray(c.projectIDs)&&c.projectIDs.length&&Array.isArray(c.capabilities)&&c.capabilities.every(v=>['read','manage','run','report'].includes(v))&&typeof c.revoked==='boolean'),'Damaged controller grants. Restore a backup.');
 ...
 create(input,endpoint){
  ...
  assert(Array.isArray(input.capabilities)&&input.capabilities.includes('read')&&input.capabilities.every(c=>['read','manage','run','report'].includes(c)),'Choose valid controller capabilities.');
 ...
 // Workbench-owned grant for the coordinator lane. It is listed and revocable like any other grant;
 // a revoked grant is never silently recreated here.
 ensureInternal(kind,name,projectIDs,capabilities,endpoint,{create=false}={}){
  const current=this.data.controllers.find(c=>c.internal===kind&&!c.revoked);
  if(!current){if(!create||!projectIDs.length)return null;const created=this.create({name,projectIDs,capabilities},endpoint);const c=this.data.controllers.find(c=>c.id===created.id);c.internal=kind;this.persist();return c;}
  const ids=[...new Set(projectIDs)];
  if(ids.length&&(ids.length!==current.projectIDs.length||ids.some(id=>!current.projectIDs.includes(id)))){current.projectIDs=ids;current.version++;this.persist();}
  return current;
 }
 // Keep an internal grant's private credential file pointed at the running server.
 refreshEndpoint(id,endpoint){const file=path.join(this.credentials,id+'.json'),saved=JSON.parse(readFileSync(file,'utf8'));if(saved.endpoint!==endpoint){writeFileSync(file+'.tmp',JSON.stringify({...saved,endpoint}),{mode:0o600});chmodSync(file+'.tmp',0o600);renameSync(file+'.tmp',file);}}
 internal(kind){return this.data.controllers.find(c=>c.internal===kind&&!c.revoked)||null;}
