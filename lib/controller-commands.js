...
export class ControllerCommands{
 constructor({controllers,store,workflows,playbooks,workspaceTasks,codex,lifecycle,mandates,threads,agents=()=>null,workbenchIssues=null}){Object.assign(this,{agents,controllers,store,workflows,playbooks,workspaceTasks,codex,lifecycle,mandates,threads,workbenchIssues});this.pending=new Set();this.closed=false;this.recover();}
 ...
 async execute(c,name,a){
  ...
  // Hub routing: the orchestrator reads project instructions and hands work to live project agents,
  // which report back. Grants still limit every call to the controller's projects and capabilities.
  if(name==='get_project_instructions'){const p=store.project(a.projectID),{files}=await instructionFiles(p.folderPath);let budget=12000;
   return {projectID:p.id,name:p.name,folderPath:p.folderPath,files:files.filter(f=>f.content).slice(0,8).map(f=>{const text=f.content.slice(0,Math.max(0,Math.min(4000,budget)));budget-=text.length;return {name:f.name,text,truncated:text.length<f.content.length};}).filter(f=>f.text)};}
  if(['get_project_agent','message_project_agent','stop_project_agent','report_to_orchestrator'].includes(name)){
   const agents=this.agents();assert(agents,'Project agents are unavailable.',503);const p=store.project(a.projectID);
   if(name==='get_project_agent')return agents.projectAgentView(p.id);
   if(name==='message_project_agent')return agents.messageProjectAgent(p,a.text,a.requestKey,c);
   if(name==='stop_project_agent')return agents.stopProjectAgent(p.id);
   return agents.reportToOrchestrator(p,a.kind,a.text,a.requestKey,c);
  }
  ...
  if(['list_coordinator_messages','post_coordinator_message','create_workbench_issue'].includes(name))assert(store.snapshot().projects.filter(p=>p.folderPath).every(p=>c.projectIDs.includes(p.id)),'The coordinator conversation requires a grant for every connected project.',403);
  if(name==='create_workbench_issue'){assert(this.workbenchIssues,'Filing Workbench issues is unavailable.',503);return this.workbenchIssues.create(a);}
