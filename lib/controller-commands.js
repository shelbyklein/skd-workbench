...
export class ControllerCommands{
 constructor({controllers,store,workflows,playbooks,workspaceTasks,codex,lifecycle,mandates}){Object.assign(this,{controllers,store,workflows,playbooks,workspaceTasks,codex,lifecycle,mandates});this.pending=new Set();this.closed=false;this.recover();}
 ...
 async preview(a){const f=this.flow(a),p=this.store.project(a.projectID);assert(p.version===a.projectVersion&&f.version===a.flowVersion,'Project or workflow changed. Refresh before previewing.',409);
  const input={...a.input,flowVersion:a.flowVersion},context=await this.workflows.preview(input,f,p),source=await benchmarkContext(p,input.mode);
  ...
  const previewToken=fingerprint({input,context,source:sourceProof,project:p,flow:f});
  ...
  return {previewToken,context,source,input:launchInput};
 }
 async execute(c,name,a){
  ...
  if(name==='preview_run'){const p=await this.preview(a);return {previewToken:p.previewToken,context:p.context,source:p.source};}
  ...
  if(name==='start_run'){
   ...
   setImmediate(async()=>{try{
    ...
    const r=await workflows.start(preview.input,this.flow(a),store.project(a.projectID),{controllerOrigin:origin,beforePersist:async()=>{
     assert(!this.closed,'Workbench is stopping.',409);controllers.check(c,'run',a.projectID);
