...
import {inspectFolder,canonicalFolder} from './projects.js';
...
export class CodexRuns{
 constructor(directory,{binary=codexBinary(),discover=()=>discoverCodex(binary),spawnProcess=spawn,timeoutMs=600000}={}){
  this.directory=directory;this.binary=binary;this.discover=discover;this.spawnProcess=spawnProcess;this.timeoutMs=timeoutMs;this.active=null;this.starting=false;this.owner=null;this.onFinish=()=>{};
 ...
 async start(input,project,internal={}){
  ...
  assert(typeof input.task==='string'&&input.task.trim()&&input.task.length<=(internal.workflowID?128000:16000),'Enter a task (up to 16,000 characters).');
  ...
  try{
   ...
   const context=internal.workspace?.sourceContext||await inspectFolder(project.folderPath);
   ...
   const r={id:randomUUID(),projectID:project.id,projectSnapshot:structuredClone(project),sourceContext:context,task:input.task.trim(),model:input.model,effort:input.effort,mode:input.mode,status:'preparing',createdAt:timestamp(),finishedAt:null,output:'',activity:[],usage:null,cost:null,error:null,threadID:null,cliVersion:provider.version,workingDirectory:context.folderPath,branch:null,...(internal.workspace||{}),workflowID:internal.workflowID||null,attemptID:internal.attemptID||null};
 ...
 finish(id,status,error){this.update(id,r=>{r.status=status;r.error=error;r.finishedAt=timestamp();});if(this.active?.id===id)this.active=null;this.onFinish(this.get(id));}
