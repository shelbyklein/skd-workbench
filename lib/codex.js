...
export class CodexRuns{
 constructor(directory,{binary=codexBinary(),discover=()=>discoverCodex(binary),spawnProcess=spawn,timeoutMs=600000}={}){
  this.directory=directory;this.binary=binary;this.discover=discover;this.spawnProcess=spawnProcess;this.timeoutMs=timeoutMs;this.active=null;this.starting=false;
  mkdirSync(directory,{recursive:true});this.file=path.join(directory,'codex-runs.json');this.runs=existsSync(this.file)?JSON.parse(readFileSync(this.file,'utf8')):[];assert(Array.isArray(this.runs),'Damaged Codex history. Restore a backup.');
  let changed=false;for(const r of this.runs)if(activeStates.includes(r.status)){r.status='interrupted';r.error='The server stopped before this run finished. It has not been restarted.';r.finishedAt=timestamp();changed=true;}
 ...
 async start(input,project){
  assert(!this.active&&!this.starting,'Another Codex task is already running. Stop or finish it first.',409);
  assert(typeof input.task==='string'&&input.task.trim()&&input.task.length<=16000,'Enter a task (up to 16,000 characters).');
  assert(['read-only','worktree'].includes(input.mode),'Choose read-only or isolated worktree.');
  assert(project.folderPath,'Connect a project folder before running Codex.');
  this.starting=true;
  try{
   const provider=await this.discover(),model=provider.models.find(m=>m.id===input.model);
   assert(model&&model.efforts.includes(input.effort),'Choose a supported Codex model and effort.');
   const context=await inspectFolder(project.folderPath);
   if(input.mode==='worktree')assert(context.git?.status==='connected'&&context.git.commit&&context.git.dirty===false,'Coding requires a clean Git repository with a commit. Commit or stash changes first.');
   const r={id:randomUUID(),projectID:project.id,projectSnapshot:structuredClone(project),sourceContext:context,task:input.task.trim(),model:input.model,effort:input.effort,mode:input.mode,status:'preparing',createdAt:timestamp(),finishedAt:null,output:'',activity:[],usage:null,cost:null,error:null,threadID:null,cliVersion:provider.version,workingDirectory:context.folderPath,branch:null};
 ...
 async execute(r){
  ...
  if(r.mode==='worktree'){
   const base=path.join(this.directory,'worktrees');mkdirSync(base,{recursive:true});const destination=path.join(base,r.id),branch='codex/skd-'+r.id;
  ...
  if(active.cancelled){clearTimeout(active.killTimer);this.finish(r.id,'cancelled','Stopped before Codex launched.');return;}
  const args=['exec','--json','--ephemeral','--ignore-user-config','--ignore-rules','--color','never','--sandbox',r.mode==='worktree'?'workspace-write':'read-only','-c','approval_policy="never"','-c','features.multi_agent=false','-c',`model_reasoning_effort="${r.effort}"`,'--model',r.model,'--cd',r.workingDirectory];
  if(r.sourceContext.git?.status!=='connected')args.push('--skip-git-repo-check');args.push('-');
  const child=this.spawnProcess(process.execPath,[fileURLToPath(new URL('../scripts/codex-child.mjs',import.meta.url)),this.binary,...args],{stdio:['pipe','pipe','pipe'],detached:true,env:{...process.env}});active.child=child;
  ...
  const consume=line=>{
   let e;try{e=JSON.parse(line);}catch{providerError='Codex emitted an invalid JSON event.';return;}
   this.update(r.id,x=>{
    if(e.type==='thread.started')x.threadID=e.thread_id;
    if(e.type==='item.completed'&&e.item?.type==='agent_message')x.output+=(x.output?'\n\n':'')+String(e.item.text||'');
    if(e.type==='item.started'||e.type==='item.completed'){const item=e.item||{};x.activity.push({at:timestamp(),type:item.type||'event',status:item.status||e.type.slice(5),text:String(item.command||item.text||item.type||'').slice(0,1000)});x.activity=x.activity.slice(-100);}
  ...
  const settle=async(code,error)=>{
   ...
   const status=active.interrupted?'interrupted':active.cancelled?'cancelled':error||code!==0||!completed||providerError?'failed':'completed';
   ...
   this.finish(r.id,status,active.interrupted?'Local server stopped.':active.cancelled?'Stopped by you.':error?.message||providerError||(status==='failed'?stderr.trim().slice(-3000)||'Codex exited without a completed turn.':null));
  };
 ...
 stop(id){const r=this.get(id);assert(this.active?.id===id&&activeStates.includes(r.status),'This run is no longer active.',409);this.active.cancelled=true;kill(this.active.child);const a=this.active;a.killTimer=setTimeout(()=>kill(a.child,'SIGKILL'),2000);return structuredClone(this.update(id,r=>{r.status='stopping';}));}
