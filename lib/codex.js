...
export class CodexRuns{
 ...
 async start(input,project,internal={}){
  const agent=input.agent||'codex';assert(['codex','claude'].includes(agent),'Choose Codex or Claude.');assert(!internal.workflowID||agent==='codex'||internal.delegationID===internal.workflowID,'Claude workflows are not supported yet.');
  ...
  try{
   ...
   assert(!(internal.workflowID||internal.issueProposal)||(!input.agentProfile&&!input.playbook),'Agent specializations are not supported for workflows, delegation or issue proposals.');
