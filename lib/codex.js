...
export class CodexRuns{
 constructor(directory,{binary=codexBinary(),discover=()=>discoverCodex(binary),spawnProcess=spawn,timeoutMs=600000,claudeOptions={},skills=null,connections=null}={}){
  this.claudeBinary=claudeOptions.binary||claudeBinary();this.discoverClaude=claudeOptions.discover||(()=>discoverClaude(this.claudeBinary));this.directory=directory;this.binary=binary;this.discover=discover;this.spawnProcess=spawnProcess;this.timeoutMs=timeoutMs;this.skills=skills;this.connections=connections;this.active=null;this.starting=false;this.cleaning=0;this.owner=null;this.onFinish=()=>{};
 ...
 async start(input,project,internal={}){
  ...
  try{
   ...
   const skillContext=internal.agentContext?.skills?(this.skills?.validateSnapshot(project,agent,internal.agentContext.skills)||internal.agentContext.skills):(this.skills?.resolve(project,agent,input.skills||{mode:'inherit',skillIDs:[]},{purpose})||{status:'empty',entries:[],text:''});
   const connectionContext=this.connections?await this.connections.resolve(project,agent,input.connections||{mode:'inherit',connectionIDs:[]},{purpose,mode:input.mode}):{status:'excluded',connections:[],reason:'Connections are unavailable.'};delete connectionContext._connections;
   const r={id:randomUUID(),agent,projectID:project.id,projectSnapshot:structuredClone(project),sourceContext:context,task:input.task.trim(),model:input.model,effort:input.effort,mode:input.mode,status:'preparing',createdAt:timestamp(),finishedAt:null,output:'',activity:[],usage:null,cost:null,error:null,threadID:null,cliVersion:provider.version,workingDirectory:context.folderPath,branch:null,...(internal.workspace||{}),purpose:internal.issueProposal?'issue-proposal':null,workflowID:internal.workflowID||null,attemptID:internal.attemptID||null,agentContext:{skills:skillContext,connections:connectionContext}};
