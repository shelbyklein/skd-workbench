...
import {WorktreeNotes} from './worktree-notes.js';
...
import {assert,Problem,toolFreePurposes} from './domain.js';
...
export function codexArgs(r){
 let args=['exec','--json','--ephemeral','--ignore-user-config','--ignore-rules','--color','never','--sandbox',r.mode==='worktree'?'workspace-write':'read-only','-c','approval_policy="never"','-c','features.multi_agent=false','-c',`model_reasoning_effort="${r.effort}"`,...(deliveredInstructions(r.agentContext)?['-c',`developer_instructions=${JSON.stringify(deliveredInstructions(r.agentContext))}`]:[]),'--model',r.model,'--cd',r.workingDirectory];
 if(toolFreePurposes.includes(r.purpose))args.push('--disable','shell_tool','--disable','unified_exec','-c','web_search="disabled"');
 if(r.sourceContext?.git?.status!=='connected')args.push('--skip-git-repo-check');
 args.push('-');
 return args;
}
export async function git(folder,args){const env={...process.env,GIT_TERMINAL_PROMPT:'0'};for(const k of Object.keys(env))if(k.startsWith('GIT_')&&k!=='GIT_TERMINAL_PROMPT')delete env[k];return (await exec('git',['-c','core.hooksPath=/dev/null','-c','core.fsmonitor=false','-C',folder,...args],{env,timeout:15000,maxBuffer:1024*1024})).stdout;}
export class CodexRuns{
 constructor(directory,{binary=codexBinary(),discover=()=>discoverCodex(binary),spawnProcess=spawn,timeoutMs=600000,claudeOptions={},skills=null,connections=null,playbooks=null,workspaceNotes=null}={}){
  this.claudeBinary=claudeOptions.binary||claudeBinary();this.discoverClaude=claudeOptions.discover||(()=>discoverClaude(this.claudeBinary));this.directory=directory;this.binary=binary;this.discover=discover;this.spawnProcess=spawnProcess;this.timeoutMs=timeoutMs;this.skills=skills;this.connections=connections;this.playbooks=playbooks;this.active=null;this.starting=false;this.cleaning=0;this.projectAgentBusy=()=>false;this.owner=null;this.onFinish=()=>{};
  mkdirSync(directory,{recursive:true});this.directory=realpathSync(directory);this.workspaceNotes=workspaceNotes||new WorktreeNotes(this.directory);this.file=path.join(directory,'codex-runs.json');this.runs=existsSync(this.file)?JSON.parse(readFileSync(this.file,'utf8')):[];assert(Array.isArray(this.runs),'Damaged Codex history. Restore a backup.');
 ...
 // A live project agent owns its project folder; other agent work there waits until it stops.
 projectFree(projectID){assert(!projectID||!this.projectAgentBusy(projectID),'The project agent is working in this project. Stop it in the project conversation before starting another session or workflow here.',409);}
 async start(input,project,internal={}){
  const agent=input.agent||'codex';assert(['codex','claude'].includes(agent),'Choose Codex or Claude.');
  ...
  this.projectFree(project?.id);
  assert(!this.active&&!this.starting&&!this.cleaning,'Another agent session is already running. Stop or finish it first.',409);
  assert(typeof input.task==='string'&&input.task.trim()&&input.task.length<=(internal.workflowID||internal.issueProposal||internal.briefing?128000:16000),'Enter a task (up to 16,000 characters).');
  ...
  try{
   ...
   assert(!(internal.workflowID||internal.issueProposal||internal.briefing)||(!input.agentProfile&&!input.playbook),'Agent specializations are not supported for workflows, delegation, issue proposals or briefings.');
   const purpose=internal.issueProposal?'issue-proposal':internal.briefing?'daily-briefing':internal.workflowID?'workflow':'session';
   const playbookContext=!internal.agentContext&&purpose==='session'&&this.playbooks?await this.playbooks.resolveLaunch(project,agent,profileSelection(input),{purpose,mode:input.mode}):internal.agentContext?.agentProfile||internal.agentContext?.playbook||null;
   if(internal.workflowID&&playbookContext)this.playbooks?.validateSnapshot(project,agent,playbookContext);
   ...
   if(internal.shouldLaunch)assert(internal.shouldLaunch(),'Workflow stopped before this step launched.',409);
   const r={id:randomUUID(),agent,projectID:project.id,projectSnapshot:structuredClone(project),sourceContext:context,task:input.task.trim(),model:input.model,effort:input.effort,mode:input.mode,status:'preparing',createdAt:timestamp(),finishedAt:null,output:'',activity:[],toolActivity:createToolActivity(agent),usage:null,cost:null,error:null,threadID:null,cliVersion:provider.version,workingDirectory:context.folderPath,branch:null,...(internal.workspace||{}),purpose:internal.issueProposal?'issue-proposal':internal.briefing?'daily-briefing':null,workflowID:internal.workflowID||null,delegationID:internal.delegationID||null,attemptID:internal.attemptID||null,agentContext:{agentProfile:playbookContext?structuredClone(playbookContext):null,skills:skillContext,connections:connectionContext}};
   deliveredInstructions(r.agentContext);
   if(playbookContext?.status==='selected'&&!internal.agentContext)r.agentProfileSelection=structuredClone(profileSelection(input));
   r.workspaceOwnerKey=internal.workspaceOwnerKey||internal.workflowID||'session:'+r.id;
   r.workspacePurpose=(internal.workspacePurpose??input.task).trim().slice(0,240);
   r.workspaceWorkRef=internal.workRef||'local:'+(internal.workflowID||r.id);
 ...
 async execute(r){
  ...
  if(r.mode==='worktree'&&!r.worktreePath){
   ...
   const registration=this.workspaceNotes.prepare({intentID:r.id,projectID:r.projectID,sourceContext:r.sourceContext,destination,branch,purpose:r.workspacePurpose,origin:{kind:r.workflowID?'workflow':'session',id:r.workflowID||r.id},workRef:r.workspaceWorkRef,ownerKey:r.workspaceOwnerKey,classification:r.projectSnapshot.benchmark?'benchmark':'development'});
   this.update(r.id,x=>{x.workspaceRegistrationID=registration.id;x.worktreePath=destination;x.branch=branch;x.workingDirectory=path.join(destination,path.relative(r.sourceContext.git.root,r.sourceContext.folderPath));});
   if(active.interrupted||active.cancelled){if(!active.interrupted)this.finish(r.id,'cancelled','Stopped before workspace creation.');return;}
   await git(r.sourceContext.git.root,['worktree','add','-b',branch,destination,r.sourceContext.git.commit]);
   const attached=await this.workspaceNotes.attach(r.id);
   this.update(r.id,x=>{x.workspaceRegistration=attached;});
  ...
  if(r.worktreePath){
   assert(r.workspaceRegistrationID,'This legacy workspace has no verified registration. Start a new task; the existing workspace is retained.',409);
   const verified=await this.workspaceNotes.verify(r.workspaceRegistrationID,r.worktreePath,r.workspaceOwnerKey);
   this.update(r.id,x=>{x.workspaceRegistration=verified;});
   this.workspaceNotes.link(r.workspaceRegistrationID,{kind:'session',id:r.id},r.workspaceOwnerKey);
   if(active.interrupted)return;
   if(active.cancelled){this.finish(r.id,'cancelled','Stopped before agent launched.');return;}
  }
  if(r.workflowID){this.playbooks?.validateSnapshot(r.projectSnapshot,r.agent,r.agentContext?.agentProfile);if(r.agentContext?.skills)this.skills?.validateSnapshot(r.projectSnapshot,r.agent,r.agentContext.skills);}
  let args=codexArgs(r);
  if(r.agent==='claude')args=claudeArgs(r);
  const env={...process.env};if(toolFreePurposes.includes(r.purpose))for(const key of ['GH_TOKEN','GITHUB_TOKEN','GH_ENTERPRISE_TOKEN','GITHUB_ENTERPRISE_TOKEN'])delete env[key];
  if(r.agentProfileSelection)await this.playbooks.resolveLaunch(r.projectSnapshot,r.agent,r.agentProfileSelection,{purpose:'session',mode:r.mode});
 ...
 finish(id,status,error){this.update(id,r=>{r.status=status;r.error=error;r.finishedAt=timestamp();});if(this.active?.id===id)this.active=null;
  const r=this.get(id);if(r.projectSnapshot?.benchmark&&!r.workflowID&&status!=='interrupted'&&r.worktreePath&&r.workspaceRegistration?.state==='attached'){this.resetWorkspace(id,r,r,x=>this.update(id,y=>{y.reset=x;}));}
  this.onFinish(this.get(id));
 }
 async resetWorkspace(id,workspace,evidence,save){
  this.cleaning++;save({status:'saving'});
  let artifact;
  try{await this.workspaceNotes.verify(workspace.workspaceRegistrationID,workspace.worktreePath,workspace.workspaceOwnerKey);artifact=await archiveWorkspace(this.directory,id,workspace,evidence);save({status:'clearing',artifact});const result=await removeWorkspace(this.directory,workspace,artifact);save({...result,artifact});}
