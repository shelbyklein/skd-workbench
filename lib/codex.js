...
export class CodexRuns{
 constructor(directory,{binary=codexBinary(),discover=()=>discoverCodex(binary),spawnProcess=spawn,timeoutMs=600000,claudeOptions={}}={}){
 ...
 async start(input,project,internal={}){
  ...
  try{
   ...
   const r={id:randomUUID(),agent,projectID:project.id,projectSnapshot:structuredClone(project),sourceContext:context,task:input.task.trim(),model:input.model,effort:input.effort,mode:input.mode,status:'preparing',createdAt:timestamp(),finishedAt:null,output:'',activity:[],usage:null,cost:null,error:null,threadID:null,cliVersion:provider.version,workingDirectory:context.folderPath,branch:null,...(internal.workspace||{}),workflowID:internal.workflowID||null,attemptID:internal.attemptID||null};
 ...
 async execute(r){
  ...
  let args=['exec','--json','--ephemeral','--ignore-user-config','--ignore-rules','--color','never','--sandbox',r.mode==='worktree'?'workspace-write':'read-only','-c','approval_policy="never"','-c','features.multi_agent=false','-c',`model_reasoning_effort="${r.effort}"`,'--model',r.model,'--cd',r.workingDirectory];
  if(r.sourceContext.git?.status!=='connected')args.push('--skip-git-repo-check');args.push('-');
  if(r.agent==='claude')args=claudeArgs(r);
  const child=this.spawnProcess(process.execPath,[fileURLToPath(new URL('../scripts/codex-child.mjs',import.meta.url)),r.agent==='claude'?this.claudeBinary:this.binary,...args],{stdio:['pipe','pipe','pipe'],detached:true,cwd:r.workingDirectory,env:{...process.env}});active.child=child;
  ...
  const consume=line=>{
   let e;try{e=JSON.parse(line);}catch{providerError='Agent emitted an invalid JSON event.';return;}
   this.update(r.id,x=>{
